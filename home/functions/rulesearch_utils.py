...
def get_ports_of(services, service_object):
    ...
    if service_object in services:        
        if services[service_object]['svcObjType']=='1':
            ...
            elif services[service_object]['svcObjPort1'] in ['mobile-registration-request', 'mobile-host-redirect', 'datagram-error', 'traceroute', 'address-mask-reply', 'address-mask-request', 'info-reply', 'info-request', 'timestamp-reply', 'timestamp', 'parameter-problem']:
                services[service_object]['svcObjPort1']='99'
                services[service_object]['svcObjPort2']='99'

            if services[service_object]['svcObjPort2']=='echo-request':  ## needed to fix Port2 value for icmp objects read in via sonicwall API
            ...
            elif services[service_object]['svcObjPort2'] in ['mobile-registration-request', 'mobile-host-redirect', 'datagram-error', 'traceroute']:
                services[service_object]['svcObjPort2']='99'                
            try:
            ...
            return list(range(int(services[service_object]['svcObjPort1']), int(services[service_object]['svcObjPort2'])+1))
        ...
        for context in contextnames:
            ...
            if 'policies' in config[context]:
                for policy in config[context]['policies']:
                    ...
                    if (config[context]['config']['fw_type']=='checkpoint' and config[context]['policies'][policy]['policyName'] in options.policynames) or config[context]['config']['fw_type']!='checkpoint' or options.policynames[0].lower() in  ['', 'any', 'all']:
                                ...
                                for source_index in config[context]['policies'][policy]['policySrcNet']:
                                    ...
                                    if source_index not in excluded_addresses:
                                        if (source_index in config[context]['addresses']):
                                            for expanded_index in expand_address(config[context]['addresses'],config[context]['addresses'][source_index]['addrObjId'],config[context]['addressmappings']):
                                                if (expanded_index in config[context]['addresses']):
                                                    policyIPv4_list.extend(config[context]['addresses'][expanded_index]['IPv4Networks'])
                                            ...
                                            for expanded_index in expand_address(shared['addresses'],shared['addresses'][source_index]['addrObjId'],shared['addressmappings']):
                                                policyIPv4_list.extend(shared['addresses'][expanded_index]['IPv4Networks'])
                                                prefix='*'
                                        else: 
                                            ...
                                            try:
                                                if re.findall('-',source_index)!=[]:
                                                    ...
                                                    for x in ipaddress.summarize_address_range(ipaddress.IPv4Address(first),ipaddress.IPv4Address(last)):
                                    ...
                                    if excluded_src_networks & polSet == IPSet([]):
                                        if (srcSet & polSet) or ( (source_index.lower() == 'any' or source.lower() == '0.0.0.0/0') and options.zero_network):
                                            if srcSet==polSet: source_match_type='Exact'
                                            elif (srcSet & polSet)==srcSet: source_match_type='Complete'
                                            elif (srcSet & polSet)==polSet: source_match_type='Partial'
                                            elif (source_index.lower() == 'any' or source.lower() == '0.0.0.0/0'): source_match_type='Any'
                                            else: source_match_type='Mixed'
                                            found_in_source = True
                                            source_addr = config[context]['policies'][policy]['policySrcNet']
                                    ...
                                    for dest_index in config[context]['policies'][policy]['policyDstNet']:
                                        ...
                                        if dest_index not in excluded_addresses:
                                            if (dest_index in config[context]['addresses']):
                                                for expanded_index in expand_address(config[context]['addresses'],config[context]['addresses'][dest_index]['addrObjId'],config[context]['addressmappings']):
                                                    if (expanded_index in config[context]['addresses']):
                                                        policyIPv4_list.extend(config[context]['addresses'][expanded_index]['IPv4Networks'])
                                                    elif (expanded_index in shared['addresses']):
                                                        policyIPv4_list.extend(shared['addresses'][expanded_index]['IPv4Networks'])
                                                ...
                                                for expanded_index in expand_address(shared['addresses'],shared['addresses'][dest_index]['addrObjId'],shared['addressmappings']):
                                                    policyIPv4_list.extend(shared['addresses'][expanded_index]['IPv4Networks'])
                                                    prefix='*'
                                            #else:
                                                #
                                            else: 
                                                if dest_index.lower() not in ['any', '']:  log('UNKNOWN DEST in policy {} "{}"'.format(config[context]['policies'][policy]['policyName'], dest_index))
                                                try:
                                        ...
                                        if excluded_dst_networks & polSet == IPSet([]):
                                            if (polSet & destSet) or ( (dest_index.lower() == 'any' or dest.lower() == '0.0.0.0/0') and options.zero_network):
                                                if destSet==polSet: dest_match_type='Exact'
                                                elif (destSet & polSet)==destSet: dest_match_type='Complete'
                                                elif (destSet & polSet)==polSet: dest_match_type='Partial'
                                                elif  (dest_index.lower() == 'any' or dest.lower() == '0.0.0.0/0'): dest_match_type='Any'
                                                else: dest_match_type='Mixed'
                                                found_in_dest = True
                                                dest_addr = config[context]['policies'][policy]['policyDstNet']
                            ...
                            if found_in_dest:
                                ...
                                if (config[context]['policies'][policy]['policyDstSvc'] == [''] and options.zero_service) or ([x.lower() for x in config[context]['policies'][policy]['policyDstSvc']] == ['any'] and options.zero_service) or config[context]['policies'][policy]['policyDstSvc'] == ['application-default']:
                                    ...
                                    for dest_index in config[context]['policies'][policy]['policyDstSvc']:
                                        if (dest_index in config[context]['services']):
                                            for expanded_index in expand_service(config[context]['services'],config[context]['services'][dest_index]['svcObjId'],config[context]['servicemappings']):
                                                ...
                                                ## svcPortSet FIX
                                                try:
                                                    if ((prot.lower() == policy_prot or prot.lower()=='any') and (int(portnum) in policy_ports)) or (dest_index.lower() == 'any' and options.zero_network) or (service.lower() == 'any/any' and options.zero_network):
                                        ...
                                        if (dest_index in shared['services']):
                                            for expanded_index in expand_service(shared['services'],shared['services'][dest_index]['svcObjId'],shared['servicemappings']):
                                                ...
                                                #if start_port=='': start_port='0'
                                                #if end_port=='': end_port='0'
                                                if ((prot.lower() == policy_prot or prot.lower()=='any') and portnum in policy_ports) or dest_index.lower() == 'any' or (service.lower() == 'any/any' and options.zero_network):
                                                    if found_in_service == False:
                                                        found_in_service = True
                                                        dest_service = config[context]['policies'][policy]['policyDstSvc']
                                                        break
                                                            
                            if found_in_source and found_in_dest and found_in_service and (options.matchtypes in [['all'], ['any']] or (source_match_type.lower() in [x.lower() for x in options.matchtypes] or source_match_type.lower() == 'any') and (dest_match_type.lower() in [x.lower() for x in options.matchtypes] or dest_match_type.lower() == 'any' )):
                                ...
                                if config[context]['policies'][policy]['policySrcZone'] == [] or config[context]['policies'][policy]['policySrcZone'] == ['']:
                                    source_zones = ['any']
                                else:
                                    source_zones = config[context]['policies'][policy]['policySrcZone']
                                
                                if config[context]['policies'][policy]['policyDstZone'] == [] or config[context]['policies'][policy]['policyDstZone'] == ['']:
                                    dest_zones = ['any']
                                else:
                                    dest_zones = config[context]['policies'][policy]['policyDstZone']
                                
                                if config[context]['policies'][policy]['policySrcNet'] == [] or config[context]['policies'][policy]['policySrcNet'] == ['']:
                                    source_nets = ['any']
                                else:
                                    source_nets = config[context]['policies'][policy]['policySrcNet']
                                
                                if config[context]['policies'][policy]['policyDstNet'] == [] or config[context]['policies'][policy]['policyDstNet'] == ['']:
                                    dest_nets = ['any']
                                else:
                                    dest_nets = config[context]['policies'][policy]['policyDstNet']
                                
                                if config[context]['policies'][policy]['policyDstSvc'] == [] or config[context]['policies'][policy]['policyDstSvc'] == ['']:
                                    dest_services = ['any']
                                else:
                                ...
                                if options.html:
                                    ...
                                    with codecs.open(options.csv, 'a+', 'utf-8') as outfile:
                                        if policymatches==1:  ## this is to print a header line
                                            if config[context]['config']['fw_type']=='checkpoint':
                                                outfile.write('Context,Enabled,Action,PolicyName,Section,UiNum,IndexNum,Source Address,Destination Address,Service,Comment,UUID\n')
                                                
                                            else:
                                                outfile.write('Context,Enabled,Action,Name,Source Zone,Dest Zone,Source Address,Destination Address,Service,Comment,UUID\n')
                                    ...
                                    #with codecs.open('/dev/stdout', 'w', 'utf-8') as outfile:
                                        ...
                                        if config[context]['config']['fw_type']=='checkpoint':
                                            ...
                                            outfile.write('"{}",'.format(str(config[context]['policies'][policy]['policyUiNum'])))
                                            outfile.write('"{}",'.format(str(config[context]['policies'][policy]['policyNum'])))
                                        #else:
                                    ...
                                    if policymatches==1:
                                        log('Context : ' + context)
                                        if config[context]['config']['fw_type']=='checkpoint':
                                            log ('{:2.2s} | {:8.8s} | {:15.15s} | {:30.30s} | {:8.8s} | {:8.8s} | {:10.10s} | {:60.60s} | {:10.10s} | {:40.40s} | {:40.40s}'.format('En', 'Action', 'CMA', 'Policy Name', 'Rule UI#', 'Rule Idx', 'Src_match', 'Source Address', 'Dst_match', 'Destination Address', 'Service'))
                                        else:
                                            log ('{:2.2s} | {:8.8s} | {:30.30s} | {:20.20s} | {:20.20s} | {:10.10s} | {:60.60s} | {:10.10s} | {:40.40s} | {:40.40s}'.format('En', 'Action', 'Rule Name', 'Source Zone', 'Destination Zone', 'Src_match', 'Source Address', 'Dst_match', 'Destination Address', 'Service'))
                                        log ('=' * 250)
                                    #if config[context]['config']['fw_type']=='checkpoint':
                                    ...
                                    longestval = max(len(source_addr), len(dest_addr), len(dest_service), len(source_zone), len(dest_zone))
                                    #if longestval>1:
                                    for index in range(0, longestval):
                                        ...
                                        if index==0:
                                            if config[context]['config']['fw_type']=='checkpoint':
                                                log ('{:2.2s} | {:8.8s} | {:15.15s} | {:30.30s} | {:8.8s} | {:8.8s} | {:10.10s} | {:1.1s}{:60.60s} | {:10.10s} | {:1.1s}{:40.40s} | {:40.40s}'.format(enabled, action, context, url_unquote(name), str(config[context]['policies'][policy]['policyUiNum']), str(config[context]['policies'][policy]['policyNum']), str(source_match_type), srcprefix, url_unquote(source_addr[0]), str(dest_match_type), dstprefix, url_unquote(dest_addr[0]), url_unquote(dest_service[0])))
                                            else:
                                                log ('{:2.2s} | {:8.8s} | {:30.30s} | {:20.20s} | {:20.20s} | {:10.10s} | {:1.1s}{:60.60s} | {:10.10s} | {:1.1s}{:40.40s} | {:40.40s}'.format(enabled, action, url_unquote(name), url_unquote(source_zone[0]), url_unquote(dest_zone[0]), str(source_match_type), srcprefix, url_unquote(source_addr[0]), str(dest_match_type), dstprefix, url_unquote(dest_addr[0]), url_unquote(dest_service[0])))
                                        else:
                                            if config[context]['config']['fw_type']=='checkpoint':
                                                log ('{:2.2s} | {:8.8s} | {:15.15s} | {:30.30s} | {:8.8s} | {:8.8s} | {:10.10s} | {:1.1s}{:60.60s} | {:10.10s} | {:1.1s}{:40.40s} | {:40.40s}'.format('', '', '', '', '', '', '', srcprefix, url_unquote(tmpsrc), '', dstprefix, url_unquote(tmpdst), url_unquote(tmpsvc)))
                                            else:
                                                log ('{:2.2s} | {:8.8s} | {:30.30s} | {:20.20s} | {:20.20s} | {:10.10s} | {:1.1s}{:60.60s} | {:10.10s} | {:1.1s}{:40.40s} | {:40.40s}'.format('', '', '', tmpszone, tmpdzone, '', str(source_match_type), srcprefix, url_unquote(tmpsrc), '', str(dest_match_type), dstprefix, url_unquote(tmpdst), url_unquote(tmpsvc)))
                                    log ('-' * 250)
                                    if modify:
                                        if modify_group not in config[context]['policies'][policy]['policyDstNet']:
                                            for addr in modify_addr:
                                                log('addelement fw_policies {} rule:{}:dst:\'\' network_objects:{}'.format(url_unquote(name),config[context]['policies'][policy]['policyNum'],addr))
                                        else:
