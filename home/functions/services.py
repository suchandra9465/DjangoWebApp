        ...
        if options.grouptargets:
            ...
            for target, new_addresses, existing_addresses, members_added, members_existed in results:
                if new_addresses != 'Exception':
                    ...
                    print('{},{},{},{}'.format(target, 'New Addresses',
                                               len(new_addresses),
                                               new_addresses))
                    print('{},{},{},{}'.format(target, 'Existing Addresses',
                                               len(existing_addresses),
                                               existing_addresses))
                    print('{},{},{},{}'.format(target, 'New Group Members',
                                               len(members_added),
                                               members_added))
                    print('{},{},{},{}'.format(target,
                    ...
                    print('{},{},{}'.format(target, 'Exception',
                                            new_addresses))

        else:
            print(options.grouptargets)
            print(
                'Creating bulk objects without target group targets specified')
            utils.bulk_create_addresses(None, self.config, self.params)

    # todo: contexts and config
    def service_ruleSearch(self, options):
        rulesearch_utils.find_matching_rules2(self.config,
                                              self.config['shared'],
                                              options.rulematch, self.contexts,
                                              options, options.rulemodify)
    ...
    def dump_config(self, config, contexts):
        ...
        for context in contexts:
            ...
            if len(list(config[context]['policies'].keys())) > 0:
                for col, key in enumerate(config[context]['policies'][list(
                        config[context]['policies'].keys())[0]]):
            ...
            if len(list(config[context]['addresses'].keys())) > 0:
                for col, key in enumerate(config[context]['addresses'][list(
                        config[context]['addresses'].keys())[0]]):
                    xl_addresses.write(0, col, key)
            xl_services.write(0, 0, 'Services')
            row = 1
            if len(list(config[context]['services'].keys())) > 0:
                for col, key in enumerate(config[context]['services'][list(
                        config[context]['services'].keys())[0]]):
            ...
            for policy in config[context]['policies']:
                ...
                for src in config[context]['policies'][policy]['policySrcNet']:
                    if src not in config[context][
                            'addresses'] and src in config['shared'][
                                'addresses']:
                        if src not in sh_addr:
                            sh_addr.append(src)
                for dst in config[context]['policies'][policy]['policyDstNet']:
                    if dst not in config[context][
                            'addresses'] and dst in config['shared'][
                                'addresses']:
                        if dst not in sh_addr:
                            sh_addr.append(dst)
                for svc in config[context]['policies'][policy]['policyDstSvc']:
                    if svc not in config[context][
                            'services'] and svc in config['shared']['services']:
                ...
                for col, key in enumerate(config[context]['policies'][policy]):
                    ...
                    if type(config[context]['policies'][policy][key]) == list:
                        for index, item in enumerate(
                                config[context]['policies'][policy][key]):
                            # output += item
                            if key in [
                                    'policySrcNet', 'policyDstNet',
                                    'policyDstSvc'
                            ] and item in ['']:
                                output += 'any'
                            else:
                                output += item
                            if index < len(config[context]['policies'][policy]
                                           [key]) - 1:
                                output += '\n'
                    ...
                    xl_policies.write(row, col, dumpConfig_utils.ss(output),
                                      cell_format)
                row += 1
            ...
            for map in config[context]['addressmappings']:
                ...
                xl_addressgroups.write(row, 0, dumpConfig_utils.ss(map),
                                       cell_format)
                output = ''
                for index, item in enumerate(
                        config[context]['addressmappings'][map]):
                    output += item
                    if index < len(
                            config[context]['addressmappings'][map]) - 1:
                        output += '\n'
            ...
            if 'shared' in config:
                for map in config['shared']['addressmappings']:
                    if map in sh_addr:
                        ...
                        xl_addressgroups.write(row, 0,
                                               dumpConfig_utils.ss(map),
                                               cell_format)
                        output = ''
                        for index, item in enumerate(
                                config['shared']['addressmappings'][map]):
                            output += item
                            if index < len(config['shared']['addressmappings']
                                           [map]) - 1:
                                output += '\n'

                        xl_addressgroups.write(row, 1,
                                               dumpConfig_utils.ss(output),
                                               cell_format)
                        row += 1
            ...
            for map in config[context]['servicemappings']:
                ...
                xl_servicegroups.write(row, 0, dumpConfig_utils.ss(map),
                                       cell_format)
                output = ''
                for index, item in enumerate(
                        config[context]['servicemappings'][map]):
                    output += item
                    if index < len(
                            config[context]['servicemappings'][map]) - 1:
                        output += '\n'
                xl_servicegroups.write(row, 1, dumpConfig_utils.ss(output),
                                       cell_format)
                row += 1
            if 'shared' in config:
                for map in config['shared']['servicemappings']:
                    if map in sh_svc:
                        ...
                        output = ''
                        for index, item in enumerate(
                                config['shared']['servicemappings'][map]):
                            output += item
                            if index < len(config['shared']['servicemappings']
                                           [map]) - 1:
                                output += '\n'
                        if row % 2 == 1:
                            cell_format = workbook.add_format({
            ...
            for address in config[context]['addresses']:
                for col, key in enumerate(
                    ...
                    if key == 'addrObjType':
                        ...
                        output = str(
                            config[context]['addresses'][address][key])
                    if row % 2 == 1:
                        cell_format = workbook.add_format({
                            'bg_color': '#DDDDDD',
                            'text_wrap': True
                    ...
                    xl_addresses.write(row, col, dumpConfig_utils.ss(output),
                                       cell_format)
                row += 1
            if 'shared' in config:
                for address in sh_addr:
                    if address in config['shared']['addresses']:
                        for col, key in enumerate(
                                config['shared']['addresses'][address]):
                            if key == 'addrObjType':
                                if config['shared']['addresses'][address][
                                        key] == '1':
                                    output = 'Host'
                                elif config['shared']['addresses'][address][
                                        key] == '2':
                                    output = 'Range'
                                elif config['shared']['addresses'][address][
                                        key] == '4':
                                    ...
                                    output = config['shared']['addresses'][
                                        address][key]
                            elif key == 'addrObjProperties':
                                if config['shared']['addresses'][address][
                                        key] == '14':
                                ...
                                output = str(config['shared']['addresses']
                                             [address][key])
                            if row % 2 == 1:
                                cell_format = workbook.add_format({
                                    'bg_color':
                            ...
                            xl_addresses.write(row, col, utils.ss(output),
                                               cell_format)
                        row += 1
            row = 1
                ...
                for col, key in enumerate(
                    ...
                    if key == 'svcObjType':
                        ...
                        if key in config[context]['services'][service]:
                            output = str(
                                config[context]['services'][service][key])
                        pass
                    if row % 2 == 1:
                        cell_format = workbook.add_format({
                            'bg_color': '#DDDDDD',
                    ...
                    xl_services.write(row, col, dumpConfig_utils.ss(output),
                                      cell_format)
                row += 1
            if 'shared' in config:
                for service in sh_svc:
                    if service in config['shared']['services']:
                        for col, key in enumerate(
                                config['shared']['services'][service]):
                            if key == 'svcObjType':
                                if config['shared']['services'][service][
                                        key] == '1':
                                    output = 'Service'
                                elif config['shared']['services'][service][
                                        key] == '2':
                                    output = 'Service Group'
                                else:
                                    output = config['shared']['services'][
                                        service][key]
                            elif key == 'svcObjProperties':
                                if config['shared']['services'][service][
                                        key] == '14':
                                ...
                                output = str(
                                    config['shared']['services'][service][key])
                            if row % 2 == 1:
                                cell_format = workbook.add_format({
                                    'bg_color':
                                    '#AAAAAA',
