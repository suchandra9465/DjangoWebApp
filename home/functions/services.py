...
def update_status(job_id, status):
    obj = Large.objects.get(id=job_id)
    obj.status = status
    obj.save()
...
class Services:
    ...
    def service_nexpose(self, options, job_id):
            ...
            if options.grouptargets:
                ...
                for target, new_addresses, existing_addresses, members_added, members_existed in results:
                    if new_addresses != 'Exception':
                        data_entry = JobLog(job_id=job_id,
                                            name=target + "_" + new_addresses,
                                            ip=new_addresses,
                                            status="Success")
                        data_entry.save()
    ...
    def service_rule_search(self, options, job_id):
        try:
    ...
    # dump config
    def dump_config(self, config, contexts, job_id):
        try:
            sh_addr = []
            sh_svc = []
    ...
    def service_migration(self, options, job_id):
