    ...
    def bulk_create_addresses(target,options,config=None):
        ...
        try:
            ...
            for context in options.context:
                ...
                #log(context)
                address_group_members=sonicwall_utils.expand_address(config[context]['addresses'], options.nexpose, config[context]['addressmappings'])
                    ...
                    if context in config:
                        ...
                        if fw_type in ['pano', 'panorama']:
                            ...
                            if options.nexpose.lower() not in [x.lower() for x in config['shared']['addresses']] and not options.readonly:    ## create address group if needed
                                print('Creating Shared Address Group : {}'.format(options.nexpose))
                                result=sonicwall_utils.exec_fw_command(target, fw_type, [('create_address', {'addressname': options.nexpose, 'addresstype': '8', 'zone': 'LAN', 'color': 'black', 'comment': comment, 'members': [], 'context': target_context})], syntax=api_type, delay=nexpose_delay)
                            ...
                            if fw_type in ['sonicwall', 'sw65']:
                                ...
                                if not options.skipzone:
                                    for address in options.groupaddresses:
                                        if len(address.split('%'))==2:
                                            target_zone=sonicwall_utils.get_zone(target_context, address.split(',')[0].split('%')[1], config)
                                        else:
                                            target_zone=sonicwall_utils.get_zone(target_context, address.split(',')[0], config)
                                        if target_zone!=None:
                                    ...
                                    if target_zone==None:
                                        ...
                                        #log(options.groupaddresses[0].split(',')[0])
                                        try:
                                            print('Trying to determine zone for {}'.format(options.groupaddresses[0].split(',')[0]))
                                            target_zone=sonicwall_utils.get_zones2(target_context, options.groupaddresses[0].split(',')[0], config)[0]
                                        except:
                                            target_zone=None
                                ...
                                sw_objects=sonicwall_utils.get_sw_objects(target, options.username, options.password, fw_type)
                            else:
                ...
                if target_zone and (len(config[target_context]['addresses']) > 1 or context== 'shared' ):
                    ...
                    for address_to_add in options.groupaddresses:  ## build addresses_to_add
                        ...
                        if address_to_add in config[target_context]['addresses']:
                            ...
                            print('Using existing object name with exact name match {}'.format(address_to_add))
                            ...
                            print('!-- Skipping entry {} - Invalid format - Expected network/mask,address_name'.format(address_to_add))
                    ...
                    for network, mask, address_name, address_type in addresses_to_add: ## build a list of existing address objects that match each object that needs to be created
                        if address_type=='network':
                            ...
                            if matches[network_mask]['address_ip']:
                                print('Using existing object name with address_ip match {} instead of requested name {}'.format(matches[network_mask]['address_ip'], address_name))
                                group_members.append(matches[network_mask]['address_ip'])
                            elif matches[network_mask]['fqdn']:
                                print('Using existing object name with fqdn match {} instead of requested name {}'.format(matches[network_mask]['fqdn'], address_name))
                                group_members.append(matches[network_mask]['fqdn'])
                            elif matches[network_mask]['hostname']:
                                print('Using existing object name with hostname match {} instead of requested name {}'.format(matches[network_mask]['hostname'], address_name))
                                group_members.append(matches[network_mask]['hostname'])
                            elif matches[network_mask]['other']:
                                print('Using existing object name with first match {} instead of requested name {}'.format(matches[network_mask]['other'], address_name))
                                group_members.append(matches[network_mask]['other'])
                            else: ## no matches found
                                print('Creating new address object {} defined as {}'.format(new_address_name, network_mask))
                                new_addresses.append(new_address_name)
                                group_members.append(new_address_name)
                                if mask=='32':
                                    address_cmds.append(('create_address', {'addressname': new_address_name, 'ip1': network, 'ip2' : sonicwall_utils.cidr_to_netmask(mask), 'addresstype': '1', 'zone': target_zone, 'color': 'black', 'comment': comment, 'context': target_context}))
                                else:
                                    address_cmds.append(('create_address', {'addressname': new_address_name, 'ip1': network, 'ip2' : sonicwall_utils.cidr_to_netmask(mask), 'addresstype': '4', 'zone': target_zone, 'color': 'black', 'comment': comment, 'context': target_context}))
                            ...
                            if network in [config[target_context]['addressesfqdn'][x]['addrObjFqdn'] for x in config[target_context]['addressesfqdn']] and address_name in [config[target_context]['addressesfqdn'][y]['addrObjFqdnId'] for y in config[target_context]['addressesfqdn']]:
                                for y in config[target_context]['addressesfqdn']:
                                    print('{} -- {} -- {} -- {}'.format(network, config[target_context]['addressesfqdn'][y]['addrObjFqdn'], address_name, config[target_context]['addressesfqdn'][y]['addrObjFqdnId']))
                                    if network in config[target_context]['addressesfqdn'][y]['addrObjFqdn'] and address_name == config[target_context]['addressesfqdn'][y]['addrObjFqdnId']:
                                        ...
                                        print('Using existing fqdn object {} with name {}'.format(y, config[target_context]['addressesfqdn'][y]['addrObjFqdnId']))
                                        break
                                #existing_addresses.append(address_name)
                                #group_members.append(new_address_name)
                                
                            else:
                                print('Creating new fqdn object {}'.format(address_name))
                                new_address_name=address_name
                    ...
                    if not options.readonly:
                        if address_cmds != []:
                            #log(target, fw_type)
                            result=sonicwall_utils.exec_fw_command(target, fw_type, address_cmds, syntax=api_type, delay=nexpose_delay, sw_objects=sw_objects)
                            print('Creating Address objects', result)
                            
                        else:
                            print('No new addresses need to be created')
                        members_added=[]
                        members_existed=[]
                        group_created=False
                            ...
                            for member in [x for x in sublist]: # cant use sublist and then change it in the loop.
                                if options.nexpose in config[target_context]['addressmappings']:
                                    if member in config[target_context]['addressmappings'][options.nexpose]:
                                        ...
                                        print('Removing {} from sublist'.format(member))
                            ...
                            if options.nexpose.lower()!='none':
                                ...
                                while sublist != [] and result != True and tries<len(sublist):
                                    ...
                                    if options.nexpose.lower() in [x.lower() for x in config[target_context]['addresses']] or options.nexpose.lower() in [x.lower() for x in config[target_context]['addressesV6']] or group_created:
                                        result=sonicwall_utils.exec_fw_command(target, fw_type, [('modify_address', {'action': 'addmembers', 'addressname': options.nexpose, 'addresstype': '8', 'zone': target_zone, 'color': 'black', 'comment': comment, 'members': sublist, 'context': target_context})], syntax=api_type, delay=nexpose_delay, sw_objects=sw_objects)
                                        print('Adding members to existing group :', result)
                                    else:
                                        result=sonicwall_utils.exec_fw_command(target, fw_type, [('create_address', {'addressname': options.nexpose, 'addresstype': '8', 'zone': target_zone, 'color': 'black', 'comment': comment, 'members': sublist, 'context': target_context})], syntax=api_type, delay=nexpose_delay, sw_objects=sw_objects)
                                        print('Creating group and adding members :', result)
                                        if result==True:
                                            group_created=True
                                    ...
                                    if result != True:
                                        ...
                                        if fw_type=='sw65':
                                            try:
                                                ...
                                                print('Removing {} from group members'.format(bad_object))
                                                print('Group members {}'.format(sublist))
                                            except:
                                                print('Removing {} from group failed'.format(bad_object))
                                        else:
                                            #log(result)
                                            result=True
                                    else:
                    ...
                    print('!-- Unable to determine Address object zone - skipping')        
