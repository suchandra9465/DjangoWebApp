    ...
    try:
        ...
        if target is None:
            ...
            if options.fwtype in ['sw', 'sonicwall']:  ## not really supported as I need to read in routing table
                ...
                palo_xml = sonicwall_utils.get_palo_config_https(target, 'config.panorama.temp', options.username,
                                                                 options.password)
                if palo_xml:
        ...
        for context in options.context:
            ...
            # log(context)
            address_group_members = sonicwall_utils.expand_address(config[context]['addresses'], options.nexpose,
            ...
            if len(config[context][
                       'addresses']) > 1 or context == 'shared':  # at least one address must exist to continue, otherwise loading config likely failed.
                debug('addresses > 1')
                ...
                if context in config:
                    ...
                    if fw_type in ['pano', 'panorama']:
                        ...
                        if options.nexpose.lower() not in [x.lower() for x in config['shared'][
                            ...
                            result = sonicwall_utils.exec_fw_command(target, fw_type, [('create_address', {
                                'addressname': options.nexpose, 'addresstype': '8', 'zone': 'LAN', 'color': 'black',
                                'comment': comment, 'members': [], 'context': target_context})], syntax=api_type,
                                                                     delay=nexpose_delay)
                        ...
                        if fw_type in ['sonicwall', 'sw65']:
                            ...
                            if not options.skipzone:
                                for address in options.groupaddresses:
                                    if len(address.split('%')) == 2:
                                        target_zone = sonicwall_utils.get_zone(target_context,
                                                                               address.split(',')[0].split('%')[1],
                                                                               config)
                                    else:
                                        target_zone = sonicwall_utils.get_zone(target_context,
                                                                               address.split(',')[0], config)
                                    if target_zone != None:
                                ...
                                if target_zone == None:
                                    ...
                                    # log(options.groupaddresses[0].split(',')[0])
                                    try:
                                        print('Trying to determine zone for {}'.format(
                                            options.groupaddresses[0].split(',')[0]))
                                        target_zone = sonicwall_utils.get_zones2(target_context,
                            ...
                            sw_objects = sonicwall_utils.get_sw_objects(target, options.username, options.password,
                                                                        fw_type)
                        else:
            ...
            if target_zone and (len(config[target_context]['addresses']) > 1 or context == 'shared'):
                ...
                for address_to_add in options.groupaddresses:  ## build addresses_to_add
                    ...
                    if address_to_add in config[target_context]['addresses']:
                        ...
                        print('!-- Skipping entry {} - Invalid format - Expected network/mask,address_name'.format(
                            address_to_add))
                ...
                for network, mask, address_name, address_type in addresses_to_add:  ## build a list of existing address objects that match each object that needs to be created
                    if address_type == 'network':
                        ...
                        if matches[network_mask]['address_ip']:
                            print(
                                'Using existing object name with address_ip match {} instead of requested name {}'.format(
                            ...
                            print(
                                'Using existing object name with fqdn match {} instead of requested name {}'.format(
                                    matches[network_mask]['fqdn'], address_name))
                            group_members.append(matches[network_mask]['fqdn'])
                        elif matches[network_mask]['hostname']:
                            print(
                                'Using existing object name with hostname match {} instead of requested name {}'.format(
                            ...
                            print(
                                'Using existing object name with first match {} instead of requested name {}'.format(
                                    matches[network_mask]['other'], address_name))
                            group_members.append(matches[network_mask]['other'])
                        else:  ## no matches found
                            print('Creating new address object {} defined as {}'.format(new_address_name,
                                                                                        network_mask))
                            new_addresses.append(new_address_name)
                            group_members.append(new_address_name)
                            if mask == '32':
                                address_cmds.append(('create_address',
                                                     {'addressname': new_address_name, 'ip1': network,
                                                      'ip2': sonicwall_utils.cidr_to_netmask(mask),
                                                      'addresstype': '1', 'zone': target_zone, 'color': 'black',
                                                      'comment': comment, 'context': target_context}))
                            else:
                                address_cmds.append(('create_address',
                                                     {'addressname': new_address_name, 'ip1': network,
                                                      'ip2': sonicwall_utils.cidr_to_netmask(mask),
                                                      'addresstype': '4', 'zone': target_zone, 'color': 'black',
                                                      'comment': comment, 'context': target_context}))
                        ...
                        if network in [config[target_context]['addressesfqdn'][x]['addrObjFqdn'] for x in
                            ...
                            for y in config[target_context]['addressesfqdn']:
                                ...
                                if network in config[target_context]['addressesfqdn'][y][
                                    ...
                                    existing_addresses.append(
                                        config[target_context]['addressesfqdn'][y]['addrObjFqdnId'])
                                    print('Using existing fqdn object {} with name {}'.format(y, config[
                                        target_context]['addressesfqdn'][y]['addrObjFqdnId']))
                                    break
                            # existing_addresses.append(address_name)
                            # group_members.append(new_address_name)
                            ...
                            new_address_name = address_name
                            address_cmds.append(('create_address',
                                                 {'addressname': new_address_name, 'domain': network,
                                                  'addresstype': 'fqdn', 'zone': target_zone, 'color': 'black',
                                                  'comment': comment, 'context': target_context}))
                            group_members.append(new_address_name)
                if not options.readonly:
                    if address_cmds != []:
                        # log(target, fw_type)
                        result = sonicwall_utils.exec_fw_command(target, fw_type, address_cmds, syntax=api_type,
                                                                 delay=nexpose_delay, sw_objects=sw_objects)
                        ...
                        if options.nexpose.lower() != 'none':
                            ...
                            while sublist != [] and result != True and tries < len(sublist):
                                ...
                                if options.nexpose.lower() in [x.lower() for x in config[target_context][
                                    ...
                                    result = sonicwall_utils.exec_fw_command(target, fw_type, [('modify_address', {
                                        'action': 'addmembers', 'addressname': options.nexpose, 'addresstype': '8',
                                        'zone': target_zone, 'color': 'black', 'comment': comment,
                                        'members': sublist, 'context': target_context})], syntax=api_type,
                                                                             delay=nexpose_delay,
                                                                             sw_objects=sw_objects)
                                    print('Adding members to existing group :', result)
                                else:
                                    result = sonicwall_utils.exec_fw_command(target, fw_type, [('create_address', {
                                        'addressname': options.nexpose, 'addresstype': '8', 'zone': target_zone,
                                        'color': 'black', 'comment': comment, 'members': sublist,
                                        'context': target_context})], syntax=api_type, delay=nexpose_delay,
                                                                             sw_objects=sw_objects)
