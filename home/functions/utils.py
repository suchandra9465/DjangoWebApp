    ...
    try:
        ...
        if target is None:
            ...
            if options.fwtype in [
                ...
                palo_xml = sonicwall_utils.get_palo_config_https(
                    target, 'config.panorama.temp', options.username,
                    options.password)
                if palo_xml:
                ...
                if context in config:
                    ...
                    if fw_type in ['pano', 'panorama']:
                        ...
                        if fw_type in ['sonicwall', 'sw65']:
                            ...
                            if not options.skipzone:
                                for address in options.groupaddresses:
                                    if len(address.split('%')) == 2:
                                        target_zone = sonicwall_utils.get_zone(
                                            target_context,
                            ...
                            sw_objects = sonicwall_utils.get_sw_objects(
                                target, options.username, options.password,
                                fw_type)
                        else:
            ...
            if target_zone and (len(config[target_context]['addresses']) > 1
                    ...
                    if address_to_add in config[target_context]['addresses']:
                        ...
                        if fqdn != None:
                            addresses_to_add.append(
                                (fqdn, fqdn_ip, address_name, 'fqdn'))
                ...
                for network, mask, address_name, address_type in addresses_to_add:  ## build a list of existing address objects that match each object that needs to be created
                    if address_type == 'network':
                        ...
                        if mask != '32':
                            new_address_name = '{}-{}_{}'.format(
                                new_address_name, network, mask)
                        else:
                            new_address_name = '{}-{}'.format(
                        ...
                        if network in [
                            ...
                            for y in config[target_context]['addressesfqdn']:
                                print('{} -- {} -- {} -- {}'.format(
                                    network, config[target_context]
                                    ['addressesfqdn'][y]['addrObjFqdn'],
                            ...
                            print('Creating new fqdn object {}'.format(
                                address_name))
                            new_address_name = address_name
                            address_cmds.append(('create_address', {
                                'addressname': new_address_name,
                ...
                if not options.readonly:
                    ...
                    for sublist in [
                        ...
                        if options.nexpose.lower() != 'none':
                            # log('sublist', sublist)
                            tries = 0
                            while sublist != [] and result != True and tries < len(
                                    sublist):
                                tries += 1
                                ...
                                if options.nexpose.lower() in [
                                    ...
                                    result = sonicwall_utils.exec_fw_command(
                                        ...
                                        delay=nexpose_delay,
                                        sw_objects=sw_objects)
                                    print('Adding members to existing group :',
                                          result)
                                else:
                                    result = sonicwall_utils.exec_fw_command(
                                        target,
        ...
        return (target, new_addresses, existing_addresses, members_added,
                members_existed)
