...
def rhex(num):
    ...
    str = ""
    for j in range(4):
        str += hex_chr[(num >> (j * 8 + 4)) & 0x0F] + hex_chr[(num >> (j * 8))
                                                              & 0x0F]
    return str
...
def xor(dataArray, patternArray):
    ...
    for i in range(len(dataArray)):
        ...
        xorVal = (dat ^ pat)
        strResult = strResult + urllib.parse.quote(
            chr(xorVal), encoding="raw_unicode_escape")
...
def setEncryptSeed2(strPassPhrase, randomNumber):
    strInternalPageSeedHash = ''
    if (len(strPassPhrase) > 0):
        strInternalPageSeedHash = calcMD5_2(
            getChars(randomNumber + strPassPhrase))
        return strInternalPageSeedHash
...
def get_loglevel():
    ...
    if response:
        ...
        try:
            selected = re.findall(
                r'SELECTED.*',
                re.findall(r'priorityLogThreshold.*?</table>', response.text,
                           re.DOTALL)[0])[0]
        ...
        if response:
            ...
            try:
                ...
                #print (selected)
                value = re.sub(r'.*LOG_LOGGING_LEVEL = (.).*', r'\1', selected,
                               re.DOTALL)
...
def do_login(session, username, pwd, host, preempt=False, timeout=10):
    ...
    if sessId:
        reqdata = OrderedDict([('param1', param1), ('param2', param2),
                               ('id', id), ('sessId', sessId),
                               ('select2', 'English'), ('uName', username),
                               ('pass', sendpwd), ('digest', digest)])
        get_headers[
            'Cookie'] = 'temp=temp; SessId=' + sessId + '; PageSeed=' + pageseed + '; curUrl=systemStatusView.html; curUsr=admin'
        post_headers[
            'Cookie'] = 'SessId=' + sessId + '; PageSeed=' + pageseed + '; secure'
    ...
    try:
        ...
        #print(response.text)
        if swl_65:
            sessIdStr = re.findall(r'var sessIdStr = ".*',
                                   response.text)[0].split('"')[1]
            ...
            md = hashlib.sha1(
                binascii.unhexlify(randStr + pageSeedStr +
                                   pwdStr)).hexdigest().upper()
            sess = ''
...
def upload_firmware(host, username, password, firmwarefile):
    ...
    if do_login(session, username, password, host, True):
        ...
        response = get_url(
            session, 'https://' + host +
            '/getJsonData.json?_=1265891166962&dataSet=alertStatus')
        #print(response.text)
        resp_json = json.loads(response.text)
        active = (resp_json['svrrpNodeState'].lower() == 'active'
                  or resp_json['svrrpHaMode'].lower() == 'standalone')

        #active['svrrpNodeState'].lower()=='active')
        if active:
            fwfile = {
                'name': (firmwarefile, open(firmwarefile,
            ...
            try:
                response = session.post(
                    ...
                    timeout=3600,
                    headers=post_headers
                )  #, auth=(username, password))# , headers=post_headers ) # auth=(options.pushusername, options.pushpassword)
                do_logout(session, host)
            except requests.exceptions.RequestException as e:
                print(e)
                return False
...
def backup(host, username, password):  #, firmwarefile):
    ...
    if do_login(session, username, password, host, True):
        ...
        post_headers[
            'User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Safari/537.36'
        post_headers[
            'Referer'] = 'https://' + host + '/systemSettingsView.html'
        post_headers['Origin'] = 'https://' + host
        ...
        response = get_url(session,
                           'https://' + host + '/systemSettingsView.html')
        csrf = re.findall(r'csrfToken.*"',
                          response.text)[0].split('value=')[1].split('"')[1]
        response = get_url(
            session, 'https://' + host +
...
def get_tsr(host, username, password):  #, firmwarefile):
    ...
    if do_login(session, username, password, host, True):
        ...
        response = session.post(command,
                                verify=False,
                                data=content,
                                timeout=20)  #, headers=post_headers)

        response = get_url(session,
                           'https://' + host + '/systemToolsView.html')
        try:
            tsr_file = re.findall(r'parent.frames\["dwnldFrm"\].*',
                                  response.text)[0].split('/')[1].split('"')[0]
        except:
            pass
        #print(tsr_file)
        response = get_url(session,
