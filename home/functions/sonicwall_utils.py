        ...
        try:
            if addresses[address]['addrObjType'] == '1':  ## host
                addresses[address]['IPv4Networks'] = [
                    ipaddress.IPv4Network(addresses[address]['addrObjIp1'] +
                                          '/32')
                ]
            if addresses[address]['addrObjType'] == '2':  ## range
                addresses[address]['IPv4Networks'] = [
                    ipaddr for ipaddr in ipaddress.summarize_address_range(
                        ipaddress.IPv4Address(addresses[address]
                                              ['addrObjIp1']),
                        ipaddress.IPv4Address(addresses[address]
                                              ['addrObjIp2']))
                ]
            if addresses[address]['addrObjType'] == '4':
                bitmask = sum([
                    bin(int(x)).count("1")
                    for x in addresses[address]['addrObjIp2'].split(".")
                ])
                addresses[address]['IPv4Networks'] = [
                    ipaddress.IPv4Network(addresses[address]['addrObjIp1'] +
                                          '/' + str(bitmask))
...
def sw_enable_api(target, username, password):
    ...
    response = sw.get_url(
        session, 'https://' + target + '/systemAdministrationView.html')
    try:
        csrf = re.findall(r'csrfToken.*"',
                          response.text)[0].split('value=')[1].split('"')[1]
        postdata = {
            'csrfToken': csrf,
            'cgiaction': "none",
...
def cidr_to_netmask(prefix):
    ...
    return '.'.join([
        str((0xffffffff << (32 - int(prefix)) >> i) & 0xff)
        for i in [24, 16, 8, 0]
    ])
...
def get_sonicwall_exp(target, options):
    ...
    exp_config = get_sw_config_https(target, None, options.username,
                                     options.password)
    tmpconfig = None
    if exp_config:
        ...
        if memory_config:
            if options.logging == logging.DEBUG:
                with open('config_python_{}.txt'.format(target),
                          'w') as outfile:
...
def get_palo_config_https(host,
                          outfile,
                          username='admin',
                          password='password'):
    ...
    print("!-- Retrieving Palo Alto/Panorama configuration file from host : " +
          host)
    session = requests.Session()
    ...
    config = session.get('https://' + host +
                         ...
                         verify=False,
                         stream=True,
                         timeout=options.timeout_palo_api)
...
def sw_get_api_status(target, username, password):
    ...
    try:
        ...
        return None
    try:
        api_enabled = soup.find('input', attrs={
            'name': 'sonicOsApi_enable'
        }).has_attr('checked')
    except:
        api_enabled = False
    return api_enabled
...
def load_sonicwall_api(ip,
    ...
    if api_enabled or orig_api_enabled:
        ...
        session = requests.Session()
        session.mount('https://' + ip, sw.DESAdapter())
        loginResult = sw.do_login(session,
                                  options.username,
                                  options.password,
                                  ip,
                                  preempt=True)
        pbrResult = None
        if loginResult:
            pbrResult = sw.get_url(session,
                                   'https://' + ip + '/getRouteList.json')
        sw.do_logout(session, ip)

        url = 'https://{}/api/sonicos/auth'.format(ip)
        session.headers = OrderedDict([('User-Agent',
        ...
        if response_code == 200:
            ...
            result = session.get(url=url,
                                 data=post_data,
                                 verify=False,
                                 timeout=options.timeout_sw_api)
            debug(result.text)
            security_ipv4 = json.loads(result.text)
            debug('{:20.20} {:20.20} {}'.format(
                'Length', ' Security Policies',
                len(security_ipv4['access_rules'])))
            ...
            for access_rule in security_ipv4['access_rules']:
                ...
                sonicwall_config['policies'][rule_num][
                    'policySrcNegate'] = False
                sonicwall_config['policies'][rule_num][
                    'policyDstNegate'] = False
                sonicwall_config['policies'][rule_num][
                    'policySvcNegate'] = False
                ...
                if access_rule['ipv4']['action'].lower() == 'deny':
                    sonicwall_config['policies'][rule_num][
                        'policyAction'] = '0'
                elif access_rule['ipv4']['action'].lower() in [
                        'drop', 'discard'
                ]:
                    sonicwall_config['policies'][rule_num][
                        'policyAction'] = '1'
                ...
                sonicwall_config['policies'][rule_num][
                    'policyName'] = access_rule['ipv4']['name']
                ...
                sonicwall_config['policies'][rule_num][
                    'policyComment'] = access_rule['ipv4']['comment']
                ...
                sonicwall_config['policies'][rule_num][
                    'policyUUID'] = access_rule['ipv4']['uuid']
            ...
            if 'access_rules' in security_ipv6:
                ...
                for access_rule in security_ipv6['access_rules']:
                    ...
                    if access_rule['ipv6']['action'].lower() == 'deny':
                        sonicwall_config['policiesV6'][rule_num][
                            'policyAction'] = '0'
                    elif access_rule['ipv6']['action'].lower() in [
                            'drop', 'discard'
                    ]:
                        sonicwall_config['policiesV6'][rule_num][
                            'policyAction'] = '1'
                        ...
                        print(access_rule)
                    sonicwall_config['policiesV6'][rule_num][
                        'policySrcZone'] = [access_rule['ipv6']['from']]
                    sonicwall_config['policiesV6'][rule_num][
                        'policyDstZone'] = [access_rule['ipv6']['to']]
                    ...
                    if 'any' in access_rule['ipv6']['destination']['address']:
                        sonicwall_config['policiesV6'][rule_num][
                            'policyDstNet'] = ['']
                    elif 'name' in access_rule['ipv6']['destination'][
                            'address']:
                        sonicwall_config['policiesV6'][rule_num][
                            'policyDstNet'] = [
                    ...
                    elif 'group' in access_rule['ipv6']['destination'][
                            'address']:
                        sonicwall_config['policiesV6'][rule_num][
                            'policyDstNet'] = [
                                access_rule['ipv6']['destination']['address']
                    ...
                    sonicwall_config['policiesV6'][rule_num][
                        'policyComment'] = access_rule['ipv6']['comment']
                    ...
                    sonicwall_config['policiesV6'][rule_num][
                        'policyProps'] = ''  ## unknown for sonicwall6.5 policies
                    sonicwall_config['policiesV6'][rule_num][
                        'policyUUID'] = access_rule['ipv6']['uuid']
                    # log(json.dumps(access_rule, indent=3))
                    # log(json.dumps(sonicwall_config['policies'][rule_num], indent=3))
                    # log('-' *100)
                    rule_num_int += 1
            ...
            for nat_rule in nat_ipv4['nat_policies']:
                ...
                sonicwall_config['nat'][rule_num]['natPolicyName'] = nat_rule[
                    'ipv4']['name']
                ...
                sonicwall_config['nat'][rule_num][
                    'natPolicySrcIface'] = nat_rule['ipv4']['inbound']
                sonicwall_config['nat'][rule_num][
                    'natPolicyDstIface'] = nat_rule['ipv4']['outbound']
                if 'any' in nat_rule['ipv4']['source']:
                    ...
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSrc'] = [
                        nat_rule['ipv4']['source']['name']
                    ]
                elif 'group' in nat_rule['ipv4']['source']:
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSrc'] = [
                        nat_rule['ipv4']['source']['group']
                    ]
                else:
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSrc'] = [
                        ''
                ...
                if 'any' in nat_rule['ipv4']['service']:
                    ...
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSvc'] = [
                        nat_rule['ipv4']['service']['name']
                    ]
                elif 'group' in nat_rule['ipv4']['service']:
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSvc'] = [
                        nat_rule['ipv4']['service']['group']
                    ]
                else:
                    sonicwall_config['nat'][rule_num]['natPolicyOrigSvc'] = [
                        ''
                ...
                sonicwall_config['nat'][rule_num]['natPolicyUUID'] = nat_rule[
                    'ipv4']['uuid']
                sonicwall_config['nat'][rule_num][
                    'natPolicyComment'] = nat_rule['ipv4']['comment']
            ...
            debug('{:20.20} {:20.20} {}'.format(
                'Length', ' Addresses',
                len(addresses_ipv4['address_objects'])))
            ...
            for address in addresses_ipv4['address_objects']:
                ...
                sonicwall_config['addresses'][address_name][
                    'addrObjId'] = address_name
                sonicwall_config['addresses'][address_name][
                    'addrObjIdDisp'] = address_name
                sonicwall_config['addresses'][address_name][
                    'addrObjIp1'] = '2.2.2.2'
                sonicwall_config['addresses'][address_name][
                    'addrObjIp2'] = '3.3.3.3'
                if 'host' in address['ipv4']:
                    if 'ip' in address['ipv4']['host']:
                        sonicwall_config['addresses'][address_name][
                            'addrObjType'] = '1'
                        sonicwall_config['addresses'][address_name][
                            'addrObjIp1'] = address['ipv4']['host']['ip']
                        sonicwall_config['addresses'][address_name][
                            'addrObjIp2'] = '255.255.255.255'
                    else:
                        sonicwall_config['addresses'][address_name][
                            'addrObjType'] = '1'  # 512
                        sonicwall_config['addresses'][address_name][
                ...
                if 'range' in address['ipv4']:
                    sonicwall_config['addresses'][address_name][
                        'addrObjType'] = '2'
                    sonicwall_config['addresses'][address_name][
                        'addrObjIp1'] = address['ipv4']['range']['begin']
                    sonicwall_config['addresses'][address_name][
                        'addrObjIp2'] = address['ipv4']['range']['end']
                if 'network' in address['ipv4']:
                    if 'subnet' in address['ipv4']['network']:
                        sonicwall_config['addresses'][address_name][
                            'addrObjType'] = '4'
                        sonicwall_config['addresses'][address_name][
                            'addrObjIp1'] = address['ipv4']['network'][
                                'subnet']
                        sonicwall_config['addresses'][address_name][
                            'addrObjIp2'] = address['ipv4']['network']['mask']
                    else:
                        sonicwall_config['addresses'][address_name][
                            'addrObjType'] = '4'  # 2048
                        sonicwall_config['addresses'][address_name][
            ...
            debug('{:20.20} {:20.20} {}'.format(
                'Length', ' AddressesV6',
                len(addresses_ipv6['address_objects'])))
            # log(addresses_ipv4['address_objects'][0])
            ...
            for address in addresses_ipv6['address_objects']:
                ...
                if 'host' in address['ipv6']:
                    ...
                    if 'ip' in address['ipv6']['host']:
                        sonicwall_config['addressesV6'][address_name][
                            'addrObjIp1'] = address['ipv6']['host']['ip']
                        sonicwall_config['addressesV6'][address_name][
                            'addrObjIp2'] = '/128'
                        sonicwall_config['addressesV6'][address_name][
                            'addrObjV6PrefixLen'] = '/128'
                    else:
                        sonicwall_config['addressesV6'][address_name][
                ...
                if 'range' in address['ipv6']:
                    ...
                    sonicwall_config['addressesV6'][address_name][
                        'addrObjIp1'] = address['ipv6']['host']['ip']
                    sonicwall_config['addressesV6'][address_name][
                        'addrObjIp2'] = address['ipv6']['host']['ip']
                    sonicwall_config['addressesV6'][address_name][
                        'addrObjV6PrefixLen'] = ''
                if 'network' in address['ipv6']:
                    ...
                    if 'subnet' in address['ipv6']['network']:
                        sonicwall_config['addressesV6'][address_name][
                            'addrObjIp1'] = address['ipv6']['network'][
                                'subnet']
                        sonicwall_config['addressesV6'][address_name][
                            'addrObjIp2'] = address['ipv6']['network']['mask']
            ...
            for address_group in addresses_groups_ipv4['address_groups']:
                ...
                sonicwall_config['addresses'][address_name][
                    'addrObjIdDisp'] = address_name
                sonicwall_config['addresses'][address_name][
                    'addrObjUUID'] = address_group['ipv4']['uuid']
                sonicwall_config['addresses'][address_name][
                    'addrObjType'] = '8'
                ...
                try:
                    if 'address_object' in address_group['ipv4']:
                        for address_object in address_group['ipv4'][
                                'address_object']['ipv4']:
                            sonicwall_config['addressmappings'][
                                address_name].append(address_object['name'])
                except:
                    pass
                try:
            ...
            sonicwall_config['addresses'] = add_IPv4Network(
                sonicwall_config['addresses'])
            ...
            for address_group in addresses_groups_ipv6['address_groups']:
                ...
                sonicwall_config['addressesV6'][address_name][
                    'addrObjIdDisp'] = address_name
                sonicwall_config['addressesV6'][address_name][
                    'addrObjUUID'] = address_group['ipv6']['uuid']
                sonicwall_config['addressesV6'][address_name][
                    'addrObjType'] = '8'
                sonicwall_config['addressesV6'][address_name][
                    'addrObjZone'] = ''
                ...
                try:
                    if 'address_object' in address_group['ipv6']:
                        for address_object in address_group['ipv6'][
                                'address_object']['ipv6']:
                            sonicwall_config['addressmappings'][
                                address_name].append(address_object['name'])
            ...
            if 'address_objects' in addresses_fqdn:
                for address_fqdn in addresses_fqdn['address_objects']:
                    ...
                    try:
                        ...
                        sonicwall_config['addressesfqdn'][
                            address_name] = OrderedDict()
                        sonicwall_config['addressesfqdn'][address_name][
                            'addrObjFqdnId'] = address_fqdn['fqdn']['name']
                        sonicwall_config['addressesfqdn'][address_name][
                            'addrObjFqdnType'] = ''  # address_fqdn['fqdn']
                        sonicwall_config['addressesfqdn'][address_name][
                            'addrObjFqdnTTL'] = address_fqdn['fqdn']['dns_ttl']
                        sonicwall_config['addressesfqdn'][address_name][
                            'addrObjFqdn'] = address_fqdn['fqdn']['domain']
                        sonicwall_config['addressesfqdn'][address_name][
                            'addrObjFqdnUUID'] = address_fqdn['fqdn']['uuid']
                        if 'zone' in address_fqdn['fqdn']:
                            sonicwall_config['addressesfqdn'][address_name][
                                'addrObjFqdnZone'] = address_fqdn['fqdn'][
                                    'zone']
                        else:
                            sonicwall_config['addressesfqdn'][address_name][
                                'addrObjFqdnZone'] = ''
            ...
            debug('{:20.20} {:20.20} {}'.format(
                'Length', ' Services', len(services['service_objects'])))
            ...
            for service in services['service_objects']:
                ...
                sonicwall_config['services'][service_name][
                    'svcObjId'] = service_name
                sonicwall_config['services'][service_name][
                    'svcObjComment'] = ''
                sonicwall_config['services'][service_name][
                ...
                if 'tcp' in service:
                    ...
                    sonicwall_config['services'][service_name][
                        'svcObjIpType'] = '6'
                    sonicwall_config['services'][service_name][
                        'svcObjPort1'] = str(service['tcp']['begin'])
                    sonicwall_config['services'][service_name][
                        'svcObjPort2'] = str(service['tcp']['end'])
                    sonicwall_config['services'][service_name][
                        'svcObjUUID'] = service['uuid']
                elif 'udp' in service:
                    sonicwall_config['services'][service_name][
                        'svcObjType'] = '17'
                    sonicwall_config['services'][service_name][
                        'svcObjIpType'] = '17'
                    sonicwall_config['services'][service_name][
                        'svcObjPort1'] = str(service['udp']['begin'])
                    sonicwall_config['services'][service_name][
                        'svcObjPort2'] = str(service['udp']['end'])
                    sonicwall_config['services'][service_name][
                        'svcObjUUID'] = service['uuid']
                    ...
                    sonicwall_config['services'][service_name][
                        'svcObjIpType'] = '1'
                    sonicwall_config['services'][service_name][
                        'svcObjPort1'] = service['icmp']
                    sonicwall_config['services'][service_name][
                        'svcObjPort2'] = service['icmp']
                    sonicwall_config['services'][service_name][
                        'svcObjUUID'] = service['uuid']
            ...
            #   service_props = ['svcObjId', 'svcObjType', 'svcObjProperties', 'svcObjIpType', 'svcObjPort1', 'svcObjPort2', 'svcObjManagement', 'svcObjHigherPrecedence', 'svcObjComment']
            debug('{:20.20} {:20.20} {}'.format(
                'Length', ' Service Groups',
                len(service_groups['service_groups'])))
            ...
            for interface in interfaces_ipv4['interfaces']:
                ...
                sonicwall_config['interfaces'][interface_num][
                    'iface_name'] = interface['ipv4']['name']
                sonicwall_config['interfaces'][interface_num][
                    'iface_ifnum'] = str(interface_num)
                sonicwall_config['interfaces'][interface_num][
                    'portShutdown'] = '0'
                sonicwall_config['interfaces'][interface_num][
                    'interface_Zone'] = ''
                sonicwall_config['interfaces'][interface_num][
                    'iface_vlan_tag'] = ''
                sonicwall_config['interfaces'][interface_num][
                    'iface_static_ip'] = '0.0.0.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_static_mask'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_static_gateway'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_lan_ip'] = '0.0.0.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_lan_mask'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_lan_default_gw'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_mgmt_ip'] = '0.0.0.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_mgmt_netmask'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                    'iface_mgmt_default_gw'] = '255.255.255.0'
                sonicwall_config['interfaces'][interface_num][
                ...
                if 'comment' in interface['ipv4']:
                    sonicwall_config['interfaces'][interface_num][
                        'iface_comment'] = interface['ipv4']['comment']
                else:
                    sonicwall_config['interfaces'][interface_num][
                        'iface_comment'] = ''
            ...
            if pbrResult:
                if pbrResult.status_code == 200:
                    ...
                    for index, entry in enumerate(
                            json.loads(
                                pbrResult.text)['pbrPolicies'].split('|')[1:]):
                        # log(index, entry)
                        ...
                        if ipver == '0':  ## 0 is IPv4 - do not read IPv6 routes at this time
                            ...
                            if source in sonicwall_config['addresses']:
                                source = '{}/{}'.format(
                                    sonicwall_config['addresses'][source]
                                    ['addrObjIp1'],
                                    netmask_to_cidr(
                                        sonicwall_config['addresses'][source]
                                        ['addrObjIp2']))
                            if gateway in sonicwall_config['addresses']:
                                gateway = '{}'.format(
                                    sonicwall_config['addresses'][gateway]
                                    ['addrObjIp1'])
                            if destination in sonicwall_config['addresses']:
                                ...
                                for each_dest in expand_address(
                                        sonicwall_config['addresses'],
                                        destination,
                                        sonicwall_config['addressmappings'],
                                        inc_group=False):
                                    # log(each_dest, pbrindex)
                                    sonicwall_config['routing'][
                                        pbrindex] = OrderedDict()
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjId'] = name
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjProperties'] = properties
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjSrc'] = source
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjDst'] = '{}/{}'.format(
                                            sonicwall_config['addresses']
                                            [each_dest]['addrObjIp1'],
                                            netmask_to_cidr(
                                                sonicwall_config['addresses']
                                                [each_dest]['addrObjIp2']))
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjSvc'] = service
                                    sonicwall_config['routing'][pbrindex][
                                        'pbrObjGw'] = gateway
            ...
            if pbrResult == None:
                ...
                for route in routing_ipv4['route_policies']:
                    ...
                    sonicwall_config['routing'][route_num]['pbrObjId'] = route[
                        'ipv4']['name']
                    sonicwall_config['routing'][route_num][
                        'pbrObjProperties'] = ''

                    if 'name' in route['ipv4']['source']:
                        sonicwall_config['routing'][route_num][
                            'pbrObjSrc'] = route['ipv4']['source']['name']
                    else:
                        sonicwall_config['routing'][route_num][
                            'pbrObjSrc'] = ''
                    ...
                    if 'name' in route['ipv4']['destination']:
                        sonicwall_config['routing'][route_num][
                            'pbrObjDst'] = route['ipv4']['destination']['name']
                    elif 'group' in route['ipv4']['destination']:
                        sonicwall_config['routing'][route_num][
                            'pbrObjDst'] = route['ipv4']['destination'][
                                'group']
                    else:
                        sonicwall_config['routing'][route_num][
                            'pbrObjDst'] = ''
                    try:
                        if 'name' in route['ipv4']['gateway']:
                            sonicwall_config['routing'][route_num][
                                'pbrObjGw'] = route['ipv4']['gateway']['name']
                    ...
                    sonicwall_config['routing'][route_num][
                        'pbrObjIface'] = route['ipv4']['interface']
                    sonicwall_config['routing'][route_num][
                        'pbrObjIfaceName'] = route['ipv4']['interface']
                    sonicwall_config['routing'][route_num][
                        'pbrObjMetric'] = str(route['ipv4']['metric'])
                    sonicwall_config['routing'][route_num][
                        'pbrObjPriority'] = ''
                    sonicwall_config['routing'][route_num][
                        'pbrObjProbe'] = route['ipv4']['probe']
                    sonicwall_config['routing'][route_num][
                        'pbrObjComment'] = route['ipv4']['comment']
                    sonicwall_config['routing'][route_num][
                        'pbrObjUUID'] = route['ipv4']['uuid']
                ...
                for interface_index in sonicwall_config['interfaces']:
                    ...
                    if sonicwall_config['interfaces'][interface_index]['iface_name'] == 'X1' and \
                        ...
                        sonicwall_config['routing'][route_num][
                            'pbrObjProbe'] = ''
                        sonicwall_config['routing'][route_num][
                            'pbrObjComment'] = 'Auto-Added Default Route'
                        sonicwall_config['routing'][route_num][
                            'pbrObjUUID'] = ''
                        sonicwall_config['routing'][route_num][
                            'pbrObjSvc'] = ''
            ...
            for zone in zones['zones']:
                ...
                sonicwall_config['zones'][zone_name]['zoneObjUUID'] = zone[
                    'uuid']

            sonicwall_config['usedzones'] = []
            for interface in sonicwall_config['interfaces']:
                if sonicwall_config['interfaces'][interface][
                        'interface_Zone'] != '':
                    sonicwall_config['usedzones'].append(
                        sonicwall_config['interfaces'][interface]
                        ['interface_Zone'])
        else:
        ...
        for addr in sonicwall_config['addresses']:
            if sonicwall_config['addresses'][addr]['addrObjType'] == '8':
                ...
                for groupmember in expand_address(
                        sonicwall_config['addresses'], addr,
                        sonicwall_config['addressmappings']):
                    debug(groupmember)
                    debug(sonicwall_config['addresses'][groupmember])
                    for network in sonicwall_config['addresses'][groupmember][
                            'IPv4Networks']:
                        sonicwall_config['addresses'][addr]['IPSet'].add(
                            str(network))
    else:
        log('!-- API not enabled for target device {} Configuration not loaded.'
            .format(ip))

    sonicwall_config['config'][
        'name'] = ''  # context  ## CHANGEME (how do I get firewall name)
...
def load_xml(infile, memoryconfig=None):
        ...
        for address in address_list:
            ...
            for address_tags in address.findall('.'):
                ...
                for item in address_tags.iter():
                    if item.tag in [
                            'ip-netmask', 'ip-range', 'description', 'fqdn'
                    ]:
                        if item.tag.lower() == 'ip-netmask':
                            if re.findall('/', item.text):
                                ...
                                if mask == '32':
                                    ...
                                    return_address[current_address][
                                        'addrObjType'] = '4'
                                    return_address[current_address][
                                        'addrObjIp2'] = cidr_to_netmask(mask)
                                    return_address[current_address][
                                        'addrObjIp1'] = ipaddr
                                else:
                                    return_address[current_address][
                                ...
                                try:
                                    return_address[current_address][
                                        'IPv4Networks'] = [
                                            ipaddress.IPv4Network(item.text,
                                                                  strict=False)
                                        ]
                                except:
                                    pass
                            ...
                            return_address[current_address][
                                'addrObjIp1'] = range_start
                            return_address[current_address][
                                'addrObjIp2'] = range_end
                            return_address[current_address][
                                'addrObjType'] = '2'
                            return_address[current_address]['IPv4Networks'] = [
                                ipaddress.IPv4Network(u'255.255.255.254/32')
                            ]  ## why 255.255.255.254/32??
                            try:
                                return_address[current_address][
                                    'IPv4Networks'] = [
                                        ipaddr for ipaddr in
                                        ipaddress.summarize_address_range(
                                            ipaddress.IPv4Address(range_start),
                                            ipaddress.IPv4Address(range_end))
                                    ]
                            except:
                                pass
                            ...
                            return_address[current_address][
                                'addrObjIdDisp'] = item.text
                            return_address[current_address][
                                'addrObjIdComment'] = item.text
    ...
    def load_address_groups(address_list):
        ...
        for address in address_list:
            ...
            return_address_group[current_address][
                'addrObjId'] = current_address
            ...
            if address.find('description') != None:
                return_address_group[current_address][
                    'addrObjIdDisp'] = address.find('description').text
                return_address_group[current_address][
                    'addrObjComment'] = address.find('description').text
            for address_group_member in address.findall('./static/member'):
                addr_mappings[current_address].append(
                    address_group_member.text)
    ...
    def load_services(service_list):
        ...
        for service in service_list:
            ...
            if service.find('description') != None:
                return_service[current_service][
                    'svcObjComment'] = service.find('description').text
            ...
            if re.findall(',', port):  ## list of ports
                return_service[current_service]['svcObjPort1'], return_service[
                    current_service]['svcObjPort2'] = (None, None)
                return_service[current_service]['svcObjPortSet'] = port.split(
                    ',')
                debug('PORTSET: ', current_service,
                      return_service[current_service]['svcObjPortSet'])
                ...
                return_service[current_service]['svcObjType'] = '1'
                return_service[current_service]['svcObjPort1'], return_service[
                    current_service]['svcObjPort2'] = port.split('-')
            else:  ## Single port
                return_service[current_service]['svcObjType'] = '1'
    ...
    def load_policies(policy_list):
        ...
        for policy in policy_list:
            ...
            if not (disabled == 'yes' and options.skip_disabled):
                ...
                return_policy[policy_index][
                    'policySrcNegate'] = True if policy.find('.').findtext(
                        'negate-source') == 'yes' else False
                return_policy[policy_index][
                    'policyDstNegate'] = True if policy.find('.').findtext(
                        'negate-destination') == 'yes' else False
                return_policy[policy_index]['policySvcNegate'] = False
                return_policy[policy_index]['policyComment'] = policy.find(
                    '.').findtext('description')
                if return_policy[policy_index]['policyComment'] == None:
                    return_policy[policy_index][
                        'policyComment'] = ''  # Set Comment to blank if not found
                return_policy[policy_index]['policyLogSetting'] = policy.find(
                    '.').findtext('log-setting')
                if return_policy[policy_index]['policyLogSetting'] == None:
                    return_policy[policy_index][
                        'policyLogSetting'] = ''  # Set Log Setting to blank if not found
                return_policy[policy_index]['policyLogStart'] = policy.find(
                    '.').findtext('log-start')
                if return_policy[policy_index]['policyLogStart'] == None:
                    return_policy[policy_index][
                        'policyLogStart'] = ''  # Set Log Setting to blank if not found
                return_policy[policy_index]['policyLogEnd'] = policy.find(
                    '.').findtext('log-end')
                if return_policy[policy_index]['policyLogEnd'] == None:
                    return_policy[policy_index][
                ...
                for member in policy.findall('./to/member'):
                    return_policy[policy_index]['policyDstZone'].append(
                        member.text)
                for member in policy.findall('./from/member'):
                    return_policy[policy_index]['policySrcZone'].append(
                        member.text)
                for member in policy.findall('./source/member'):
                    return_policy[policy_index]['policySrcNet'].append(
                        member.text)
                for member in policy.findall('./destination/member'):
                    return_policy[policy_index]['policyDstNet'].append(
                        member.text)
                for member in policy.findall('./service/member'):
                    return_policy[policy_index]['policyDstSvc'].append(
                        member.text)
                for member in policy.findall('./application/member'):
                    return_policy[policy_index]['policyDstApps'].append(
                        member.text)
    ...
    def load_nat(policy_list, interfaces):
        ...
        for policy in policy_list:
            ...
            if not (disabled == 'yes' and options.skip_disabled):
                ...
                return_nat[policy_index]['natPolicyComment'] = policy.find(
                    '.').findtext('description')
                if return_nat[policy_index]['natPolicyComment'] == None:
                ...
                for member in policy.findall('./to/member'):
                    return_nat[policy_index]['natPolicyDstZone'].append(
                        member.text)
                for member in policy.findall('./from/member'):
                    return_nat[policy_index]['natPolicySrcZone'].append(
                        member.text)
                for member in policy.findall('./source/member'):
                    return_nat[policy_index]['natPolicyOrigSrc'].append(
                        member.text)
                for member in policy.findall('./destination/member'):
                    return_nat[policy_index]['natPolicyOrigDst'].append(
                        member.text)
                for member in policy.findall('./service/member'):
                    return_nat[policy_index]['natPolicyOrigSvc'].append(
                        member.text)
                ...
                if policy.find('.').findtext('source-translation'):
                    if policy.find('./source-translation').findtext(
                            'dynamic-ip-and-port'):
                        if policy.find(
                                './source-translation/dynamic-ip-and-port'
                        ).findtext('translated-address'):
                            for member in policy.findall(
                                    './source-translation/dynamic-ip-and-port/translated-address/member'
                            ):
                                return_nat[policy_index][
                                    'natPolicyTransSrc'].append(member.text)
                        if policy.find(
                                './source-translation/dynamic-ip-and-port'
                        ).findtext('interface-address'):
                            ...
                            if int_name in interfaces:
                                ...
                                return_nat[policy_index][
                                    'natPolicyTransSrc'] = 'UNKNOWN'
                            if policy.find(
                                    './source-translation/dynamic-ip-and-port/interface-address'
                            ).findtext('ip'):
                                return_nat[policy_index]['natPolicyTransSrc'] = [
                                    policy.find(
                                        './source-translation/dynamic-ip-and-port/interface-address/ip'
                                    ).text
                                ]
                    if policy.find('./source-translation').findtext(
                            'dynamic-ip'):
                        if policy.find('./source-translation/dynamic-ip'
                                       ).findtext('translated-address'):
                            for member in policy.findall(
                    ...
                    if policy.find('./source-translation').findtext(
                            'static-ip'):
                        if policy.find('./source-translation/static-ip'
                                       ).findtext('translated-address'):
                            return_nat[policy_index]['natPolicyTransSrc'] = [
                                policy.find(
                ...
                if policy.find('.').findtext('destination-translation'):
                    if policy.find('./destination-translation').findtext(
                            'translated-address'):
                        return_nat[policy_index]['natPolicyTransDst'] = [
                            policy.find(
                    ...
                    if policy.find('./destination-translation').findtext(
                        ...
                        return_nat[policy_index]['natPolicyTransSvc'] = [
                            policy.find(
                                './destination-translation/translated-port').
                            text
                        ]
                if policy.find('.').findtext(
                        'dynamic-destination-translation'):
                    log('dynamic-destination-translation set')
                    if policy.find('./dynamic-destination-translation'
                                   ).findtext('translated-address'):
                        return_nat[policy_index]['natPolicyTransDst'] = [
                            policy.find(
                                './dynamic-destination-translation/translated-address'
                            ).text
                        ]
                    if policy.find('./dynamic-destination-translation'
                                   ).findtext('translated-port'):
                        return_nat[policy_index]['natPolicyTransSvc'] = [
                            policy.find(
                                './dynamic-destination-translation/translated-port'
                            ).text
                        ]
    ...
    def load_zones(zone_list):
        ...
        for zone in zone_list:
            ...
            for interface in zone.findall('.//member'):
                ...
                return_zones[zone_name]['zoneObjMembers'].append(
                    interface.text)
    ...
    def load_variables(variable_list):
        ...
        for variable in variable_list:
            if variable:
                variables[variable.get('name')] = variable.find(
                    './/ip-netmask').text
    ...
    def load_interface(interface_base):
        ...
        if root.find(interface_base) != None:
            for interface_type in root.find(interface_base):
                for interface_names in root.findall(interface_base + '/' +
                                                    interface_type.tag +
                                                    '/entry'):
                    interface_name = interface_names.get('name')
                    return_interface[interface_name] = OrderedDict()
                    return_interface[interface_name]['iface_ifnum'] = str(
                        index)
                    return_interface[interface_name][
                        'iface_type'] = interface_type.tag
                    return_interface[interface_name][
                    ...
                    return_interface[interface_name][
                        'iface_mgmt_default_gw'] = ''
                    return_interface[interface_name]['iface_vlan_tag'] = ''
                    return_interface[interface_name]['portShutdown'] = ''
                    index += 1
                    comment = root.find(interface_base + '/' +
                                        interface_type.tag + '/entry[@name="' +
                                        interface_name + '"]/comment')
                    if comment != None:
                        return_interface[interface_name][
                            'iface_comment'] = comment.text
                    for interface in root.findall(interface_base + '/' +
                                                  interface_type.tag +
                                                  '/entry[@name="' +
                                                  interface_name + '"]'):
                        for interface_attribs in interface:
                            if interface_type.tag in [
                                    'ethernet', 'aggregate-ethernet'
                            ] and interface_attribs.tag.lower() == 'layer3':
                                ip = root.find(interface_base + '/' +
                                               interface_type.tag +
                                               '/entry[@name="' +
                                               interface_name + '"]/' +
                                               interface_attribs.tag +
                                               '/ip/entry[@name]')
                                if ip != None:
                                    if ip.get('name') in return_variables:
                                        ipname = return_variables[ip.get(
                                            'name')]
                                    else:
                                        ipname = ip.get('name')
                                    if re.findall('/', ipname):
                                        ...
                                        return_interface[interface_name][
                                            'iface_static_mask'] = cidr_to_netmask(
                                                return_interface[
                                                    interface_name]
                                                ['iface_static_mask'])
                                    else:
                                ...
                                if interface_type.tag == 'aggregate-ethernet':  # get ip addresses for sub-interfaces
                                    for sub_interfaces in root.findall(
                                            ...
                                            '"]/layer3/units/entry[@name]'):
                                        sub_interface = sub_interfaces.get(
                                            'name')
                                        return_interface[
                                            sub_interface] = OrderedDict()
                                        return_interface[sub_interface][
                                            'iface_ifnum'] = str(index)
                                        return_interface[sub_interface][
                                            'iface_type'] = interface_type.tag
                                        return_interface[sub_interface][
                                            'iface_name'] = sub_interface
                                        return_interface[sub_interface][
                                            'interface_Zone'] = ''  # this would get set when reading zones
                                        return_interface[sub_interface][
                                            'iface_comment'] = ''
                                        ...
                                        comment = root.find(
                                            ...
                                            '"]/' + interface_attribs.tag +
                                            '/units/entry[@name="' +
                                            sub_interface + '"]/comment')
                                        if ip != None:
                                            if ip.get('name'
                                                      ) in return_variables:
                                                ipname = return_variables[
                                                    ip.get('name')]
                                            else:
                                                ipname = ip.get('name')
                                            if re.findall('/', ipname):
                ...
                for interface_names in root.findall(interface_base + '/' +
                    ...
                    for interface in root.findall(interface_base + '/' +
                        ...
                        for interface_attribs in interface:
                            if interface_type.tag in [
                                    'ethernet', 'aggregate-ethernet'
                            ] and interface_attribs.tag == 'layer3':
                                ip = root.find(interface_base + '/' +
                                               interface_type.tag +
                                               '/entry[@name="' +
                                               interface_name + '"]/' +
    ...
    def load_vrouters(vrouter_base):
        ...
        if root.find(vrouter_base) != None:
            for vrouter in root.find(vrouter_base):
                ...
                if root.find(vrouter_base + '/entry[@name="' + vrouter_name +
                             '"]/routing-table/ip/static-route'):
                    for static_routes in root.find(
                            vrouter_base + '/entry[@name="' + vrouter_name +
                            '"]/routing-table/ip/static-route'):
                        static_name = static_routes.get('name')
                        return_vrouter[vrouter_name][
                            static_name] = OrderedDict()
                        # return_vrouter[vrouter_name][static_name]['nexthop']=''
                        return_vrouter[vrouter_name][static_name][
                            'destination'] = ''
                        return_vrouter[vrouter_name][static_name][
                            'metric'] = ''
                        ...
                        for vrouter_attribs in root.find(
                                vrouter_base + '/entry[@name="' +
                                vrouter_name +
                                '"]/routing-table/ip/static-route/entry[@name="'
                                + static_name + '"]'):
    ...
    if pan_config == True:  # loop through all device groups for Panorama
        for templates in root.findall('./devices/entry/template/entry'):
            ...
            variable_list = root.findall(
                './devices/entry[@name="localhost.localdomain"]/template/entry[@name=\''
                ...
                for zone in return_zones:
                    ...
                    for zone_member in return_zones[zone]['zoneObjMembers']:
                        for interface in return_interface:
                            ...
                            if return_interface[interface][
                                    'iface_name'] == zone_member:
            ...
            if options.panoramaip:
                return_config[template]['config'][
                    'mgmtip'] = options.panoramaip
            else:
        ...
        for device_groups in root.findall(
            ...
            for logs in root.findall(
                    './devices/entry/device-group/entry[@name=\'' +
                    current_group + '\']/log-settings/profiles/entry'):
                logprofiles.append(logs.get('name'))

            ##      LOAD ADDRESSES FROM XML
            log('  |-- Address Objects        \r', end=' ')
            address_list = root.findall(
                './devices/entry/device-group/entry[@name=\'' + current_group +
                '\']/address/entry')
            ...
            if address_list != []:
                tmp_addresses, addr_mappings = load_address_groups(
                    address_list)
            ...
            if options.panoramaip:
                return_config[current_group]['config'][
                    'mgmtip'] = options.panoramaip
            else:
        ...
        return_config['shared'][
            'logprofiles'] = logprofiles  # move this to 'config'
        ...
        interface_list = root.findall(
            './devices/entry[@name="localhost.localdomain"]/network/interface/ethernet'
        )
...
def exec_fw_command(
    ...
    dryrun=False,
    sw_objects=None
):  # add sw_sesssion, enable_api and commit options -- what is policy lens for?
    ...
    valid_commands = [
        'create_address', 'modify_address', 'create_rule', 'modify_rule',
        'create_service', 'modify_service', 'raw_command'
    ]
    ...
    sw_objects = None
    if fw_type.lower() in [
            'sonicwall', 'palo', 'paloalto', 'pano', 'sw65', 'checkpoint'
    ] and syntax != 'cli':
        session = requests.Session()
        session.mount('https://' + target, sw.DESAdapter())
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        if not options.web and (options.username == None
                                or options.password == None):
            options.username, options.password = get_creds()
        if fw_type.lower() == 'sonicwall':
            response = sw.do_login(session, options.username, options.password,
                                   target, True)
            apikey = None
                ...
                try:
                    ...
                    while response_code != 200 and login_tries < 1:
                        ...
                        response = session.post(
                            ...
                            headers={
                                ...
                                "Basic " + base64.b64encode('{}:{}'.format(
                                    options.username,
                                    options.password).encode()).decode()
                            },
                            verify=False,
                            timeout=options.timeout_sw_webui_login)
                        response_code = response.status_code
                    ...
                    if response_code != 200:
                        ...
                        if not sw_objects:
                            ...
                            sw_objects = get_sw_objects(
                                target, options.username, options.password,
                                fw_type, session)
                    success = True
                except Exception as e:
                    log('An exception occured when trying to log in to Sonicwall : {}'
                        .format(e))

        elif fw_type.lower() in ['palo', 'pano', 'paloalto'] and use_session:
            try:
                key = session.get(
                    'https://' + target + '/api/?type=keygen&user=' +
                    options.username + '&password=' + quote(options.password),
                    verify=False,
                    stream=True,
                    timeout=options.timeout_palo_api)
            ...
            apikey, session, message = ckpt_login(target, options.context[0],
                                                  options.username,
                                                  options.password)
            if not apikey:
                ...
                debug(
                    '!-- Login to Checkpoint R80 API successful - Retreived SID {} and UID {}'
                    .format(apikey, session))
                pass
    ...
    if apikey or fw_type.lower() not in ['palo', 'paloalto', 'pano', 'sw65'
                                         ] or syntax.lower() == 'cli':
        #
        tries = 0
        success = False
            ...
            try:
                ...
                for command, params in commands:
                    ...
                    if command == 'create_address':
                        result = create_address_obj(target, session, apikey,
                                                    fw_type, syntax, params,
                                                    sw_objects)
                    elif command == 'modify_address':
                        result = modify_address_obj(target, session, apikey,
                                                    fw_type, syntax, params,
                                                    sw_objects)
                        ...
                        result = create_rule_obj(target, session, apikey,
                                                 fw_type, syntax, params,
                                                 sw_objects)
                    elif command == 'modify_rule':
                        result = modify_rule_obj(target, session, apikey,
                                                 fw_type, syntax, params,
                                                 sw_objects)
                    elif command == 'create_service':
                        result = create_service_obj(target, session, apikey,
                                                    fw_type, syntax, params,
                                                    sw_objects)
                    elif command == 'modify_service':
                        result = modify_service_obj(target, session, apikey,
                                                    fw_type, syntax, params,
                                                    sw_objects)
                    else:
                    ...
                    if syntax.lower() != 'cli':
                        debug('{},{},{},"{}",{}'.format(
                            target, fw_type, command, params, result))
                    if delay:
                ...
                log('An exception occured when trying to perform exec_fw_command : {}'
                    .format(e))
        ...
        if successful_commands > 0:
            ...
            if fw_type.lower() in ['sw65']:
                while tries < retries and not success:
                    tries += 1
                    try:
                        commit_result = session.get(
                            'https://{}/api/sonicos/config/pending'.format(
                                target),
                            data=None,
                            verify=False,
                        ...
                        if json.loads(commit_result.text) != {}:
                            ...
                            if 'status' in json.loads(commit_result.text):
                                success = True
                            if not json.loads(
                                    commit_result.text)['status']['success']:
                                result = False, json.loads(
                                    commit_result.text
                                )['status']['info'][0]['message']
                        else:
                        ...
                        log('An exception occured when trying to commit Sonicwall config : {}'
                            .format(e))
            elif fw_type.lower() == 'checkpoint' and syntax.lower() == 'api':
                if result == True:
                    debug('result before publish', result)
                    publish_result = ckpt_api_call(target, 443, "publish", {},
                                                   apikey)
                    ...
                    if 'task-id' not in json.loads(publish_result.text):
                        ...
                        discard_result = ckpt_api_call(target, 443, "discard",
                                                       {'uid': session},
                                                       apikey)
                ...
                logout_result = ckpt_api_call(target, 443, "logout", {},
                                              apikey)
        ...
        if 'routing' in config[context]:
            ...
            for interface in config[context]['interfaces']:
                ...
                if config[context]['interfaces'][interface][
                        'iface_lan_ip'] != '0.0.0.0':
                    if ip in netaddr.IPNetwork('{}/{}'.format(
                            config[context]['interfaces'][interface]
                        ['iface_lan_ip'],
                            netmask_to_cidr(config[context]['interfaces']
                                            [interface]['iface_lan_mask']))):
                        debug(
                            'matches lan', config[context]['interfaces']
                            [interface]['interface_Zone'])
                        return config[context]['interfaces'][interface][
                            'interface_Zone']
                if config[context]['interfaces'][interface][
                        'iface_static_ip'] != '0.0.0.0':
                    ...
                    if ip in netaddr.IPNetwork('{}/{}'.format(
                        ...
                        ['iface_static_ip'],
                            netmask_to_cidr(
                                ...
                                ['iface_static_mask']))):
                        debug(
                            'matches static', config[context]['interfaces']
                            [interface]['interface_Zone'])
                        return config[context]['interfaces'][interface][
                            'interface_Zone']
                if config[context]['interfaces'][interface][
                    ...
                    if ip in netaddr.IPNetwork('{}/{}'.format(
                        ...
                        ['iface_mgmt_ip'],
                            netmask_to_cidr(
                                ...
                                ['iface_mgmt_netmask']))):
                        debug(
                            'matches mgmt', config[context]['interfaces']
                            [interface]['interface_Zone'])
                        return config[context]['interfaces'][interface][
                            'interface_Zone']
            next_hop = None
            ...
            for route in config[context]['routing']:
                ...
                if config[context]['routing'][route]['pbrObjSrc'] == "":
                    if route_dest in config[context]['addresses']:
                        ...
                        if config[context]['addresses'][route_dest][
                                'addrObjType'] == '8':
                            # log_info(config[context]['addresses'][route_dest])
                            log_info(
                                'Route Destination is a group, checking each member object'
                            )
                            for route_dest_addr in expand_address(
                                    config[context]['addresses'], route_dest,
                                    config[context]['addressmappings']):
                                if route_dest_addr in config[context][
                                        'addresses']:
                                    log_info(route_dest_addr)
                                    # print(ip)
                                    if config[context]['addresses'][
                                            route_dest_addr][
                                                'addrObjType'] == '2':
                                        route_destination = netaddr.IPRange(
                                            config[context]['addresses']
                                            [route_dest_addr]['addrObjIp1'],
                                            config[context]['addresses']
                                            [route_dest_addr]['addrObjIp2'])
                                    else:
                                        route_destination = netaddr.IPNetwork(
                                            '{}/{}'.format(
                                                config[context]['addresses']
                                                [route_dest_addr]
                                                ['addrObjIp1'], config[context]
                                                ['addresses'][route_dest_addr]
                                                ['addrObjIp2']))
                                    if netaddr.IPAddress(
                                            ip) in route_destination:
                                        ...
                                        if netmask_to_cidr(
                                            ...
                                            matchlen = netmask_to_cidr(
                                                ...
                                                [route_dest_addr]
                                                ['addrObjIp2'])
                                            next_hop = config[context][
                                                'routing'][route]['pbrObjGw']
                                            next_hop_ifacenum = config[
                                                context]['routing'][route][
                                                    'pbrObjIface']
                                            debug('Nexthop : ', next_hop)
                                            debug(config[context]['routing']
                                                  [route])
                                            if next_hop in config[context][
                                                    'addresses']:
                                                debug(
                                                    'Next hop object found in addresses'
                                                )
                                                next_hop_ip = config[context][
                                                    'addresses'][next_hop][
                                                        'addrObjIp1']
                                            else:
                                                next_hop_ip = next_hop
                                            if next_hop_ip == '':
                                                if config[context]['routing'][
                                                        route][
                                                            'pbrObjIface'] in [
                                                                config[context]
                                                                ['interfaces']
                                                    ...
                                                    for x in config[context][
                                                            'interfaces']:
                                                        if config[context]['routing'][route]['pbrObjIface'] == \
                                                                config[context]['interfaces'][x]['iface_name']:
                                                            if config[context][
                                                                    'interfaces'][
                                                                        x]['iface_lan_ip'] != '0.0.0.0':
                                                                next_hop_ip = config[
                                                                    context][
                                                                        'interfaces'][
                                                                            x]['iface_lan_default_gw']
                                                            else:
                                                                next_hop_ip = config[
                                                                    context][
                                                                        'interfaces'][
                                                                            x]['iface_static_gateway']
                                            log_info(
                                                'Searched address found in destination group: "{}" - MatchLength {} Nexthop {} {}'
                                                .format(
                                            ...
                                            log_info(
                                                'Skipping - not longest match')
                                else:
                                    log_info(
                                        'Address group not found in context')
                        elif config[context]['addresses'][route_dest][
                                'addrObjType'] == '2':
                            if netaddr.IPAddress(ip) in netaddr.IPRange(
                                    config[context]['addresses'][route_dest]
                                ['addrObjIp1'], config[context]['addresses']
                                [route_dest]['addrObjIp2']):
                                # if ip in config[context]['addresses'][route_dest]['IPSet']:
                                log_info(
                                    'Searched address found in destination range address object'
                                )
                                if netmask_to_cidr(
                                        config[context]['addresses']
                                    [route_dest]['addrObjIp2']) > matchlen:
                                    matchlen = 32
                                    next_hop = config[context]['routing'][
                                        route]['pbrObjGw']
                                    next_hop_ifacenum = config[context][
                                        'routing'][route]['pbrObjIface']
                                    if next_hop in config[context][
                                            'addresses']:
                                        next_hop_ip = config[context][
                                            'addresses'][next_hop][
                            ...
                            if netaddr.IPAddress(ip) in netaddr.IPNetwork(
                                    '{}/{}'.format(
                                        config[context]['addresses']
                                        [route_dest]['addrObjIp1'],
                                        config[context]['addresses']
                                        [route_dest]['addrObjIp2'])):
                                # if ip in config[context]['addresses'][route_dest]['IPSet']:
                                log_info(
                                    'Searched address found in destination address'
                                )
                                if netmask_to_cidr(
                                        config[context]['addresses']
                                    [route_dest]['addrObjIp2']) > matchlen:
                        ...
                        try:
                            if netaddr.IPAddress(ip) in netaddr.IPNetwork(
                                ...
                                if int(mask) >= matchlen:
                                    ...
                                    next_hop = config[context]['routing'][
                                        route]['pbrObjGw']
                                    next_hop_ifacenum = str(
                                        config[context]['routing'][route]
                                        ['pbrObjIface'])
                                    if 'pbrObjIfaceName' in config[context][
                                            'routing'][route]:
                                        next_hop_ifacename = config[context][
                                            'routing'][route][
                                                'pbrObjIfaceName']
                                    else:
                                        next_hop_ifacename = ''
                                ...
                                if next_hop in config[context]['addresses']:
                                    next_hop_ip = config[context]['addresses'][
                                        next_hop]['addrObjIp1']
                                else:
                                    next_hop_ip = next_hop
                        except Exception as e:
                        ...
                        matchlen = 0
                        next_hop = config[context]['routing'][route][
                            'pbrObjGw']
                        next_hop_ifacenum = config[context]['routing'][route][
                            'pbrObjIface']
                        if 'pbrObjIfaceName' in config[context]['routing'][
                                route]:
                            next_hop_ifacename = config[context]['routing'][
                                route]['pbrObjIfaceName']
                        else:
                            next_hop_ifacename = ''
                        if next_hop in config[context]['addresses']:
                            next_hop_ip = config[context]['addresses'][
                ...
                for interface in config[context]['interfaces']:
                    if config[context]['interfaces'][interface][
                        ...
                        if netaddr.IPAddress(next_hop_ip) in netaddr.IPNetwork(
                            ...
                            return config[context]['interfaces'][interface][
                                'interface_Zone']
                    elif config[context]['interfaces'][interface][
                            'iface_static_ip'] != '0.0.0.0':
                            ...
                            if ip in netaddr.IPNetwork('{}/{}'.format(
                                ...
                                ['iface_lan_ip'],
                                    netmask_to_cidr(
                                        config[context]['interfaces']
                                        [interface]['iface_lan_mask']))):
                                return config[context]['interfaces'][
                                    interface]['interface_Zone']
...
def get_sw_objects(target, username, password, fw_type, session=None):
    ...
    if session == None:
        ...
        if 'address_objects' in addresses_fqdn:
            for address_object in [
                    address['fqdn']['name']
                    for address in addresses_fqdn['address_objects']
            ]:
        ...
        for address_object in [
                address['ipv4']['name']
                for address in addresses_ipv4['address_groups']
        ]:
        ...
        for address_object in [
                address['ipv6']['name']
                for address in addresses_ipv6['address_groups']
        ]:
        ...
        result = session.get(url=url,
                             ...
                             timeout=options.timeout_sw_api)
        services = json.loads(result.text)
        for service_object in [
                service['name'] for service in services['service_objects']
        ]:
        ...
        services = json.loads(result.text)
        for service_object in [
                service['name'] for service in services['service_groups']
        ]:
...
def get_zones2(context, address_obj, tmpconfig=None):
    ...
    for address in expand_address(
            config[context]['addresses'], address_obj,
            config[context]['addressmappings']
    ):  # expand_address(config[context]['addresses'], route_dest, config[context]['addressmappings']):
...
def load_sonicwall(infile, skipdisabled, memoryconfig=None):
    ...
    address_props = [
        'addrObjId', 'addrObjIdDisp', 'addrObjType', 'addrObjZone',
        'addrObjProperties', 'addrObjIp1', 'addrObjIp2', 'addrObjComment'
    ]
    addressfqdn_props = [
        'addrObjFqdnId', 'addrObjFqdnType', 'addrObjFqdnZone',
        'addrObjFqdnProperties', 'addrObjFqdn'
    ]
    service_props = [
        'svcObjId', 'svcObjType', 'svcObjProperties', 'svcObjIpType',
        'svcObjPort1', 'svcObjPort2', 'svcObjManagement',
        'svcObjHigherPrecedence', 'svcObjComment'
    ]
    zone_props = ['zoneObjId', 'zoneObjComment']
    policy_props = [
        'policyAction', 'policySrcZone', 'policyDstZone', 'policySrcNet',
        'policyDstNet', 'policyDstSvc', 'policyDstApps', 'policyComment',
        'policyLog', 'policyEnabled', 'policyProps'
    ]
    interface_props = [
        'iface_ifnum', 'iface_type', 'iface_name', 'interface_Zone',
        'iface_comment', 'iface_static_ip', 'iface_static_mask',
        'iface_static_gateway', 'iface_lan_ip', 'iface_lan_mask',
        'iface_lan_default_gw', 'iface_mgmt_ip', 'iface_mgmt_netmask',
        'iface_mgmt_default_gw', 'iface_static_gateway', 'iface_vlan_tag',
        'iface_comment', 'iface_http_mgmt', 'iface_https_mgmt',
        'iface_ssh_mgmt', 'iface_ping_mgmt', 'iface_snmp_mgmt', 'portShutdown'
    ]
    routing_props = [
        'pbrObjId', 'pbrObjProperties', 'pbrObjSrc', 'pbrObjDst', 'pbrObjSvc',
        'pbrObjGw', 'pbrObjIface', 'pbrObjIfaceName', 'pbrObjMetric',
        'pbrObjPriority', 'pbrObjProbe', 'pbrObjComment'
    ]
    nat_props = [
        'natPolicyOrigSrc', 'natPolicyOrigDst', 'natPolicyOrigSvc',
        'natPolicyTransSrc', 'natPolicyTransDst', 'natPolicyTransSvc',
        'natPolicySrcIface', 'natPolicyDstIface', 'natPolicyEnabled',
        'natPolicyComment', 'natPolicyProperties', 'natPolicyName'
    ]
    addressV6_props = [
        'addrObjV6Id', 'addrObjV6IdDisp', 'addrObjV6Type', 'addrObjV6Zone',
        'addrObjV6Properties', 'addrObjV6Ip1', 'addrObjV6Ip2',
        'addrObjV6PrefixLen'
    ]
    policyV6_props = [
        'policyActionV6', 'policySrcZoneV6', 'policyDstZoneV6',
        'policySrcNetV6', 'policyDstNetV6', 'policyDstSvcV6',
        'policyCommentV6', 'policyLogV6', 'policyEnabledV6', 'policyPropsV6'
    ]
    ...
    sonicwall_config['config']['name'] = re.findall('firewallName=.*',
                                                    config)[0].split('=')[1]
    sonicwall_config['config']['version'] = re.findall(
        'buildNum=.*', config)[0].split('=')[1].split('-')[0]
    ...
    sonicwall_config['addressmappings'] = generate_group_mappings(
        config, 'addro')
    ...
    sonicwall_config['addresses'] = add_IPv4Network(
        sonicwall_config['addresses'])
    for address in sonicwall_config[
            'addresses']:  ## Add empty comment for all sonicwall address objects
    ...
    sonicwall_config['addressesfqdn'] = migrate('addrObjFqdn', config,
                                                addressfqdn_props)
    sonicwall_config['addressesV6'] = migrate('addrObjV6', config,
                                              addressV6_props)

    ## Rename IPv6 keys to match IPv4 objects
    for address in sonicwall_config['addressesV6']:
        sonicwall_config['addressesV6'][address][
            'addrObjId'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6Id')
        sonicwall_config['addressesV6'][address][
            'addrObjIdDisp'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6IdDisp')
        sonicwall_config['addressesV6'][address][
            'addrObjType'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6Type')
        sonicwall_config['addressesV6'][address][
            'addrObjZone'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6Zone')
        sonicwall_config['addressesV6'][address][
            'addrObjProperties'] = sonicwall_config['addressesV6'][
                address].pop('addrObjV6Properties')
        sonicwall_config['addressesV6'][address][
            'addrObjIp1'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6Ip1')
        sonicwall_config['addressesV6'][address][
            'addrObjIp2'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6Ip2')
        sonicwall_config['addressesV6'][address][
            'addrObjPrefixLen'] = sonicwall_config['addressesV6'][address].pop(
                'addrObjV6PrefixLen')
    ...
    for service_name in sonicwall_config[
            'services']:  ## add svcSrcPort property to all objects
    ...
    sonicwall_config['policies'] = migrate_orig('policy',
                                                config,
                                                policy_props,
                                                skipdisabled=False)
    sonicwall_config['policies'] = policy_objects_to_list(
        sonicwall_config['policies'], [
            'policySrcZone', 'policyDstZone', 'policySrcNet', 'policyDstNet',
            'policyDstSvc'
        ])

    log('!-- Generating IPv6 Policy Objects')
    sonicwall_config['policiesV6'] = migrate_orig('policy',
                                                  config,
                                                  policyV6_props,
    ...
    for policy in sonicwall_config['policiesV6']:
        sonicwall_config['policiesV6'][policy][
            'policyAction'] = sonicwall_config['policiesV6'][policy].pop(
                'policyActionV6')
        sonicwall_config['policiesV6'][policy][
            'policySrcZone'] = sonicwall_config['policiesV6'][policy].pop(
                'policySrcZoneV6')
        sonicwall_config['policiesV6'][policy][
            'policyDstZone'] = sonicwall_config['policiesV6'][policy].pop(
                'policyDstZoneV6')
        sonicwall_config['policiesV6'][policy][
            'policySrcNet'] = sonicwall_config['policiesV6'][policy].pop(
                'policySrcNetV6')
        sonicwall_config['policiesV6'][policy][
            'policyDstNet'] = sonicwall_config['policiesV6'][policy].pop(
                'policyDstNetV6')
        sonicwall_config['policiesV6'][policy][
            'policyComment'] = sonicwall_config['policiesV6'][policy].pop(
                'policyCommentV6')
        sonicwall_config['policiesV6'][policy]['policyLog'] = sonicwall_config[
            'policiesV6'][policy].pop('policyLogV6')
        sonicwall_config['policiesV6'][policy][
            'policyEnabled'] = sonicwall_config['policiesV6'][policy].pop(
                'policyEnabledV6')
        sonicwall_config['policiesV6'][policy][
            'policyProps'] = sonicwall_config['policiesV6'][policy].pop(
                'policyPropsV6')
        sonicwall_config['policiesV6'][policy][
            'policyDstSvc'] = sonicwall_config['policiesV6'][policy].pop(
                'policyDstSvcV6')

    if options.expandcheckpoint:  ## change this to an argparse option
        for policy in sonicwall_config['policies']:
            if sonicwall_config['policies'][policy]['policySrcNet'][0][
                    0:11].lower() == 'importchkpt':
                sonicwall_config['policies'][policy][
                    'policySrcNet'] = sonicwall_config['addressmappings'][
                        sonicwall_config['policies'][policy]['policySrcNet']
                        [0]]
            if sonicwall_config['policies'][policy]['policyDstNet'][0][
                    0:11].lower() == 'importchkpt':
                sonicwall_config['policies'][policy][
                    'policyDstNet'] = sonicwall_config['addressmappings'][
                        sonicwall_config['policies'][policy]['policyDstNet']
                        [0]]
            if sonicwall_config['policies'][policy]['policyDstSvc'][0][
                    0:11].lower() == 'importchkpt':
                sonicwall_config['policies'][policy][
                    'policyDstSvc'] = sonicwall_config['servicemappings'][
                        sonicwall_config['policies'][policy]['policyDstSvc']
                        [0]]
