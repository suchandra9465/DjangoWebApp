...
def get_loglevel():
    ...
    if response:
        ...
        try:
            selected = \
            re.findall(r'SELECTED.*', re.findall(r'priorityLogThreshold.*?</table>', response.text, re.DOTALL)[0])[0]
    ...
    if do_login(session, username, password, host, True):
        ...
        if active:
            ...
            content = OrderedDict([('csrfToken', (None, csrf)),
                                   ('firmware', (firmwarefile, open(firmwarefile, 'rb'), 'application/octet-stream'))])
            command = 'https://' + host + '/upload.cgi?safeMode=1'
            try:
                response = session.post(command, verify=False, files=content, timeout=3600,
                                        headers=post_headers)  # , auth=(username, password))# , headers=post_headers ) # auth=(options.pushusername, options.pushpassword)
                do_logout(session, host)
            except requests.exceptions.RequestException as e:
                print(e)
                return False
...
def backup(host, username, password):  # , firmwarefile):
    ...
    if do_login(session, username, password, host, True):
        ...
        if active:
            ...
            # print(command)
            try:
                response = session.post(command, verify=False, data=content, timeout=600,
                                        headers=post_headers)  # , auth=(username, password))# , headers=post_headers ) # auth=(options.pushusername, options.pushpassword)
                # print(response)
                # print(response.text)
                do_logout(session, host)
